...
cd /opt/client-qt
tar xf "/src/$RELEASE_TARBALL_FILENAME" client-qt/extras/packaging/gnu-linux/packaging/rules/rpm \
    --strip-components=6 && mv rpm/* . && rmdir rpm
rm jami-libqt.spec
...
if [ ! -f "${RPM_PATH}" ]; then
    ...
    (
        ...
        tar xf "/src/$RELEASE_TARBALL_FILENAME" \
            client-qt/extras/packaging/gnu-linux/packaging/rules/rpm/jami-libqt.spec \
            --strip-components=4
