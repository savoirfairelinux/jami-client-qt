...
if gh pr create --base master --head $VERSION --title "Update Jami to $VERSION" --body "Automated update to version $VERSION" --label "Jenkins"; then
    echo "Pull request created successfully!"
else
    echo "Failed to create pull request. It might already exist."
    exit 1
fi
...
if gh pr merge --merge --body "Automated merge from script"; then
    echo "Pull request merged successfully!
          ...
          Make sure to confirm its existence on the repository!"
    exit 1
else
