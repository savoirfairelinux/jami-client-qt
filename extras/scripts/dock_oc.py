...
class DockerManager:
    ...
    def run_container(self):
        ...
        result = subprocess.run([
            "docker", "run", "-d", "-t", "-u", f"{self.user_id}:{self.user_id}",
...
if __name__ == "__main__":
    ...
    if args.command == "interactive":
        # For the interactive shell, ensure the image exists or build it if necessary
        manager = DockerManager(args.image_tag, str(args.user_id), args.workspace, args.cache, args.dockerfile)
        if not manager.image_exists():
            manager.build_image()
        print("Starting an interactive shell...")
        subprocess.run([
            "docker", "run", "-it", "--rm", "-u", f"{args.user_id}:{args.user_id}",
            "-v", f"{args.workspace}:/foo:rw",
            "-v", f"{args.cache}:/var/cache/jami:rw",
            "-w", "/foo", args.image_tag, "/bin/bash"
        ])
