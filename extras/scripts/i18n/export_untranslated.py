...
def export_language(lang, out_dir):
    tslib.validate_lang(lang)
    path = ts_path(lang)
    if not os.path.isfile(path):
        raise FileNotFoundError(path)
    plurals = tslib.has_unfinished_plural(path)
    if plurals:
        raise NotImplementedError(
            '{}: {} plural message(s) need translation but this tool does not '
            'handle <numerusform> yet (e.g. {!r}). Add plural support before '
            'exporting this language.'.format(lang, len(plurals), plurals[0][1]))
