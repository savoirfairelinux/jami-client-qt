...
3. **Translate** by filling the `translation` fields in the JSON. Leave a
   ...
   later). Keep `%1`, `%2`, … placeholders and any HTML markup intact.
...
- `import_translations.py` aligns entries by document order and refuses to run
  if a `.ts` changed between export and import, so re-run the export after any
  `update-translations.py`.
- Translated text is XML-escaped to match the entities `lupdate` emits
  (`&amp; &lt; &gt; &quot; &apos;`), keeping the files canonical.
- `tslib.py` holds the shared `.ts` parsing/patching logic.
