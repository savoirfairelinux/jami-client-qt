...
- `import_translations.py` matches entries to messages by document position
  ...
  `translations/` (symlinks resolved), and each filled translation is checked
  to preserve the source's Qt placeholders (`%1`, `%L1`, `%n`).
- Translated text is XML-escaped to match the entities `lupdate` emits
  (`&amp; &lt; &gt; &quot; &apos;`), keeping the files canonical.
- Plural messages (`numerus="yes"` with `<numerusform>`) are not handled yet;
  the project currently has none, and the exporter fails loudly if one ever
  needs translating rather than skipping it silently.
