...
def _resolve_ts_path(rel_path):
    ...
    The path comes from translator-controlled JSON, so an absolute path or
    ``../`` must not be allowed to escape the translations directory.
    """
    ts_path = os.path.abspath(os.path.join(REPO_ROOT, rel_path))
