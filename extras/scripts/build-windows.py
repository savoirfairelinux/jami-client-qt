...
def run_tests(config_str, mute_jamid):
    ...
    # TODO: ctest -V -C Release --test-arguments "-mutejamid"

    output_dir = os.path.join(repo_root_dir, 'x64', config_str)
    test_qml_files_path = os.path.join(
        repo_root_dir, 'tests', 'qml', 'src')
    qml_test_cmd = os.path.join(output_dir,
                                'qml_tests.exe -input ') + test_qml_files_path
    unit_test_cmd = os.path.join(output_dir, 'unittests.exe')
