...
The MSI is built with WiX v4 (WixToolset.Sdk/4.0.5). One-time setup required:
...
def generate_msi(version, config_str="Release"):
    ...
    # WiX v4 SDK outputs to bin/<Platform>/<Configuration>/<Culture>/
    msi_dir = os.path.join(installer_dir, "bin", "x64", config_str, "en-us")
    cfg_tag = config_str.lower()  # 'release' or 'beta'
    msi_file = os.path.join(msi_dir, f"jami.{cfg_tag}.x64.msi")
...
def main():
    ...
    if parsed_args.subcommand == "pack":
        do_build(False)
        if parsed_args.msi:
            generate_msi(get_version(), config_str)
