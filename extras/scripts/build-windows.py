...
optional arguments:
  ...
  -t, --runtests        Build and run tests
  -i, --init            Initialize submodules
  -v, --version         Show the version number and exit
  -s, --skip-build      Only do packaging or testing, skip building
...
1.  build.py --init pack --msi      # Build the application from scratch and
                                      an MSI installer
2.  build.py --init --runtests      # Build the application from scratch and
                                      run tests
...
def build(config_str, qtver, tests):
    ...
    print("Generating project using cmake - config: " + config_str)
    cmd = ["cmake", ".."]
    ...
    print("Building project using cmake " + config_str)
...
def run_tests(config_str, mute_jamid):
    """Run tests."""
    print("Running client tests")

    output_dir = os.path.join(this_dir, 'x64', config_str)
    test_qml_files_path = os.path.join(
        repo_root_dir, 'tests', 'qml', 'src')
    qml_test_cmd = os.path.join(repo_root_dir, 'x64', 'test',
                                'qml_tests.exe -input ') + test_qml_files_path
    unit_test_cmd = os.path.join(repo_root_dir, 'x64', 'test', 'unittests.exe')
    ...
    def run_test(test_cmd, test_name):
        ...
        if mute_jamid:
            test_cmd += " -mutejamid"
        # output to files for jenkins
        if is_jenkins:
            test_cmd += " -o " + os.path.join(
                repo_root_dir, "x64", "test", test_name + ".txt")
        if execute_cmd(test_cmd, True):
            print("Failed when running " + test_name)
            sys.exit(1)
    ...
    # add the Release configuration output directory to the PATH
    os.environ["PATH"] += os.pathsep + os.path.join(
        repo_root_dir, 'x64', 'Release')
    # add the Qt bin directory to the PATH (for QtTest)
    os.environ["PATH"] += os.pathsep + os.path.join(qt_dir, 'bin')

    copy_tree(os.path.join(qt_dir, 'qml', 'QtTest'),
              os.path.join(output_dir, 'QtTest'))

    # run the tests
    # run_test(unit_test_cmd, "unit_tests")
    run_test(qml_test_cmd, "qml_tests")
...
def parse_args():
    ...
    parser.add_argument(
        "-t", "--runtests", action="store_true", help="Build and run tests")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Only do packaging or runtests, skip build step")
    ...
    if parsed_args.subcommand == "pack":
        ...
        if not skip_build:
            build(config_str, parsed_args.qtver,
                  parsed_args.runtests)
        if parsed_args.runtests:
            run_tests(config_str, True)
