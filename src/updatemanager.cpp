...
UpdateManager::UpdateManager(QObject* parent)
    ...
    , baseUrl_("https://dl.jami.net/windows")
...
UpdateManager::checkForUpdates(bool quiet)
{
    ...
    get(versionUrl, [this, quiet](const QString& latest) {
        if (latest.isEmpty()) {
...
UpdateManager::applyUpdates(bool beta)
{
    ...
    get(
        ...
        [this, downloadUrl](const QString&) {
            ...
            auto cmdStartInstaller = "powershell " + tempPath_ + "\\" + downloadUrl.fileName()
                                     + " /L*V " + tempPath_ + "\\jami_x64_install.log" + args;
            emit appCloseRequested();
