...
void
NetWorkManager::getFile(const QUrl& fileUrl,
                        const QString& path,
                        bool withUI,
                        std::function<void(int)> doneCbRequestInFile)
{
    if (reply_ && reply_->isRunning()) {
        qWarning() << "NetworkManager::getFile - currently downloading";
        return;
    } else if (fileUrl.isEmpty() || path.isEmpty()) {
        qWarning() << "NetworkManager::getFile - invalid parameters";
        return;
    }
    ...
    connect(reply_,
            ...
            [this, withUI, doneCbRequestInFile](QNetworkReply::NetworkError code) {
                getFileResetStatus(code, withUI, doneCbRequestInFile);
            });
    ...
    connect(reply_, &QNetworkReply::finished, [this, withUI, doneCbRequestInFile] {
        ...
        getFileResetStatus(statusCode, withUI, doneCbRequestInFile);
    });
...
void
NetWorkManager::reset(bool flushInput)
{
    reply_->deleteLater();
    reply_ = nullptr;
...
void
NetWorkManager::getString(const QUrl& fileUrl, std::function<void(int, QString)> doneCbRequest)
{
    if (reply_ && reply_->isRunning()) {
        qWarning() << Q_FUNC_INFO << "currently downloading";
        return;
    } else if (fileUrl.isEmpty()) {
        qWarning() << Q_FUNC_INFO << "fileUrl is empty";
        return;
    }
...
void
NetWorkManager::getFileResetStatus(int code,
                                   bool withUI,
...
void
NetWorkManager::getStringResetStatus(const QString& response,
                                     int code,
