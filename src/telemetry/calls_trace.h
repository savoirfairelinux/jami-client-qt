...
// Allow client-side source files to include this header without the daemon's
// PACKAGE_VERSION compile definition (defined by daemon/CMakeLists.txt only).
#ifndef PACKAGE_VERSION
#define PACKAGE_VERSION "jami-client-qt"
#endif
...
callsTracer()
{
    return opentelemetry::trace::Provider::GetTracerProvider()->GetTracer(kCallsTracerName, PACKAGE_VERSION);
}
 ...
 */
inline const char*
callStateStr(uint8_t s)
{
    static constexpr const char* names[] = {"INACTIVE", "ACTIVE", "HOLD", "BUSY", "PEER_BUSY", "MERROR", "OVER"};
 ...
 */
inline const char*
cnxStateStr(uint8_t s)
{
    static constexpr const char* names[] = {"DISCONNECTED", "TRYING", "PROGRESSING", "RINGING", "CONNECTED"};
