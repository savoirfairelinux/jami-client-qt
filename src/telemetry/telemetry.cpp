...
initTelemetry(const std::string& serviceName, const std::string& version)
{
    ...
    try {
        ...
        auto provider = std::make_unique<trace_sdk::TracerProvider>(std::move(processors), resourceAttrs);
        ...
        auto tracer = trace_api::Provider::GetTracerProvider()->GetTracer("jami.client", version);
        auto startupSpan = tracer->StartSpan("client.startup");
