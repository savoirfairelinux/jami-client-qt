...
{
    qWarning() << "updateContact 88888888888888888888888888888888888" << uri;
    ...
    if (ci != pimpl_->contacts.end()) {
        if (infos.contains("avatar")) {
            ...
            if (uri == "2ab86857e82d3e60e13e5770ac44b4268257cab7") {
                qWarning() << "========== updateContact uri " << uri;
                qWarning() << "========== updateContact avatar: " << ci->profileInfo.avatar;
            }

        } else if (!lrc::api::Lrc::cacheAvatars.load()) {
            // Else it will be reseted
            ci->profileInfo.avatar = storage::avatar(owner.id, uri);
            qWarning() << "========== resetAvatar " << uri;
        }

        if (infos.contains("title")) {
            ci->profileInfo.alias = infos["title"];
            qWarning() << "updateContact" << infos["title"];
        }
