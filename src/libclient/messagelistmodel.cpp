...
MessageListModel::editMessage(const QString& msgId, interaction::Info& info)
{
    ...
    if (it != editedBodies_.end()) {
        if (info.oldbodies.isEmpty()) {
            info.oldbodies.push_back(interaction::Body {msgId, info.body, info.timestamp});
        }
        ...
        for (const auto& editedBody : *it) {
            auto itCommit = std::find_if(info.oldbodies.begin(), info.oldbodies.end(), [&](const auto& element) {
                return element.commitId == editedBody.commitId;
            });
            if (itCommit == info.oldbodies.end()) {
