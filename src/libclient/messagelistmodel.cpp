...
MessageListModel::editMessage(const QString& msgId, interaction::Info& info)
{
    ...
    if (it != editedBodies_.end()) {
        ...
        // TODO struct to find easily if already added (because MessageReceived can be triggered
        // multiple times for same message)
        for (auto editedBody : *it) {
            auto findMsg = false;
            for (auto i = 0; i != info.oldbodies.size(); ++i) {
                if (editedBody[0] == info.oldbodies[i][0]) {
                    findMsg = true;
                    break;
                }
            }
            if (!findMsg) {
                info.oldbodies.push_back(editedBody);
            }
        }
        // TODO info.oldbodies.append(*it)
