...
MessageListModel::MessageListModel(const QString& accountUri, QObject* parent)
...
MessageListModel::dataForItem(item_t item, int, int role) const
{
    ...
    case Role::Body: {
        if (item.second.type == lrc::api::interaction::Type::CALL) {
            return QVariant(lrc::authority::storage::getCallInteractionString(item.second.authorUri == accountUri_, item.second));
        }
