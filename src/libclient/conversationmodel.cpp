...
ConversationModel::clearUnreadInteractions(const QString& convId)
{
    ...
    if (conversation.isSwarm()) {
        updated = true;
        lastDisplayedId = conversation.interactions->getLatestId();
    } else
        std::tie(updated, lastDisplayedId) = conversation.interactions->clearUnread(pimpl_->db);
...
ConversationModelPimpl::initConversations()
{
    ...
    for (auto const& c : linked.owner.contactModel->getAllContacts().toStdMap()) {
        ...
        conversations[convIdx].interactions->resolveTransferStates(db);
        auto& interactions = conversations[convIdx].interactions;
        interactions->forEachInteraction([&](const QString& id, interaction::Info& interaction) {
...
ConversationModelPimpl::sort(const conversation::Info& convA, const conversation::Info& convB)
{
    ...
    return historyA->getLatestTimestamp() > historyB->getLatestTimestamp();
}
                                            ...
                                            const SwarmMessage& message)
{
    ...
    try {
        ...
        if (!conversation.interactions->insert(msgId, msg)) {
            qWarning() << Q_FUNC_INFO << "Insert failed: duplicate ID";
            return;
        }
                                           ...
                                           const SwarmMessage& message)
{
    ...
    try {
        ...
        conversation.interactions->update(msgId, msg);
...
ConversationModelPimpl::slotPendingContactAccepted(const QString& uri)
{
    ...
    if (!convs.empty()) {
        try {
            ...
            if (convIdx >= 0) {
                conversations[convIdx].interactions->insert(msgId, interaction, 0);
            }
                                            ...
                                            bool isRequest)
{
    ...
    conversation.interactions->forEachInteraction(
        [&](const QString& id, interaction::Info& interaction) {
                                                ...
                                                int status)
{
    ...
    if (conversationId.isEmpty() || conversationId == linked.owner.profileInfo.uri) {
        ...
        QString oldDisplayedUid = 0;
        {
            ...
            //                                   && interactions->isOutgoing(msgId);

            //     // Get the last displayed interaction ID and timestamp for this peer.
            //     auto [lastIdForPeer, lastTimestampForPeer] = interactions->getDisplayedInfoForPeer(
            //         peerId);

            //     if (!lastIdForPeer.isEmpty()) {
            //         bool interactionIsLast = lastTimestampForPeer < it->second.timestamp;
            ...
            //     if (messageId != "") {
            //         // There is a displayed interaction for this peer.
            //         auto lastDisplayedIt = interactions->find(messageId);
            //         // Check if the interaction is the last displayed for this peer.
            //         bool interactionIsLast = lastDisplayedIt == interactions->end()
            //                                  || lastDisplayedIt->second.timestamp
            //                                         < it->second.timestamp;

            //         // Update the displayed interaction if necessary.
            //         updateDisplayedUid = interactionDisplayed && interactionIsLast;

            //         if (updateDisplayedUid) {
            //             // Update the displayed interaction.
            //             oldDisplayedUid = messageId;
            //             // If the displayed interaction
            //             if (peerId != linked.owner.profileInfo.uri)
            //                 conversation.interactions->setRead(peerId, it->first);
            //         }
            //     } else {
            //         // No displayed interaction for this peer yet.
            //         oldDisplayedUid = "";
            //         if (peerId != linked.owner.profileInfo.uri)
            //             conversation.interactions->setRead(peerId, it->first);
            //         updateDisplayedUid = true;
