...
getCallInteractionStringNonSwarm(bool isSelf, const std::time_t& duration)
{
    if (duration < 0) {
        ...
        if (duration) {
            return QObject::tr("Outgoing call") + " - " + QObject::tr("Lasted") + " " + getFormattedCallDuration(duration);
        } else {
            return QObject::tr("Missed outgoing call");
        }
    } else {
        if (duration) {
            return QObject::tr("Incoming call") + " - " + QObject::tr("Lasted") + " " + getFormattedCallDuration(duration);
        } else {
