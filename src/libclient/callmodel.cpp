...
public Q_SLOTS:
    ...
    /**
     * Listen from CallbacksHandler when a call arrives with media
     * @param accountId
     * @param callId
     * @param from
     * @param mediaList
                                 ...
                                 const VectorMapStringString& mediaList)
{
    ...
    if (linked.hasCall(callId)) {
        qWarning() << "slotIncomingCall: Did not initialize call" << callId << "because it already exists";
        return;
    }
    ...
    if (!linked.owner.confProperties.allowIncoming && linked.owner.profileInfo.type == profile::Type::JAMI) {
    ...
    qWarning() << "slotIncomingCall: Initialized call" << callId;
