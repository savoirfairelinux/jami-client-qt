...
CallModel::getProposed(const QString& callId, const QString& source, MediaRequestType type, bool mute)
{
    auto& callInfo = pimpl_->calls[callId];
    ...
    if (type == MediaRequestType::FILESHARING)
...
CallModel::addMedia(const QString& callId, const QString& source, MediaRequestType type, bool mute)
{
    ...
    auto proposedList = getProposed(callId, source, type, mute);
                       ...
                       bool muteCamera)
{
    ...
    if (!isConf) {
        ...
        if (!hasVideo) {
            callInfo->mediaList = proposedList;
            proposedList = getProposed(callInfo->id,
