    ...
    case MediaRequestType::FILESHARING: {
        ...
        if (shareAudio)
            audioMediaAttribute = {{MediaAttributeKey::MEDIA_TYPE, MediaAttributeValue::AUDIO},
                                   {MediaAttributeKey::ENABLED, TRUE_STR},
                                   {MediaAttributeKey::MUTED, muteMedia ? TRUE_STR : FALSE_STR},
...
void
CallModel::addMedia(const QString& callId, const QString& source, MediaRequestType type, bool muteMedia, bool shareAudio)
{
    auto& callInfo = pimpl_->calls[callId];
    if (!callInfo || source.isEmpty())
        return;

    auto proposedList = getProposed(callInfo->mediaList, callId, source, type, muteMedia, shareAudio);
