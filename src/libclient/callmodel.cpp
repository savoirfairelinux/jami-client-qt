...
private:
    ...
    void applyFallbackConversationSwitch(const QString& confId,
                                         OptRef<conversation::Info> currentConversation,
...
CallModel::replaceDefaultCamera(const QString& callId, const QString& deviceId)
{
    ...
    for (auto& media : proposedList) {
        if (media[MediaAttributeKey::MEDIA_TYPE] == MediaAttributeValue::VIDEO
            && media[MediaAttributeKey::SOURCE].startsWith(libjami::Media::VideoProtocolPrefix::CAMERA)) {
                       ...
                       bool shareAudio)
{
    ...
    // Track sources of sharing media (FILE/DISPLAY) that are being replaced
    // so we can remove their associated audio
    std::set<QString> replacedSources;
    for (auto& media : mediaList) {
        ...
        if (replace) {
            // Store the source being replaced so we can filter out associated audio
            bool isFileOrDisplay = media[MediaAttributeKey::SOURCE].startsWith(libjami::Media::VideoProtocolPrefix::FILE)
                                   || media[MediaAttributeKey::SOURCE].startsWith(
                                       libjami::Media::VideoProtocolPrefix::DISPLAY);

            if (isFileOrDisplay) {
                replacedSources.insert(media[MediaAttributeKey::SOURCE]);
            }
        ...
        // Skip this media if:
        // 1. It's a FILE source and we're requesting CAMERA (backward compatibility)
        // 2. It's audio with a replaced sharing source (e.g screenshare video got replaced by camera, so we do not add
        // the screenshare audio to the list)
        bool isAudio = media[MediaAttributeKey::MEDIA_TYPE] == MediaAttributeValue::AUDIO;
        bool isReplacedSource = replacedSources.count(media[MediaAttributeKey::SOURCE]) > 0;

        if (!(media[MediaAttributeKey::SOURCE].startsWith(libjami::Media::VideoProtocolPrefix::FILE)
              && type == MediaRequestType::CAMERA)
            && !(isAudio && isReplacedSource)) {
    ...
    // Only add screenshare audio in supported configurations:
    // - 1:1 calls only (not in conferences; conference audio negotiation not yet implemented)
    // - Not on macOS (audio capture not yet implemented in Core Audio layer)
    bool isMacOS = false;
#ifdef Q_OS_MAC
    isMacOS = true;
#endif
    if (!audioMediaAttribute.isEmpty() && !(type == MediaRequestType::SCREENSHARING && isConf) && !isMacOS)
...
CallModel::joinCalls(const QString& callIdA, const QString& callIdB) const
{
    ...
    if (call1.type == call::Type::CONFERENCE && call2.type == call::Type::CONFERENCE) {
        bool joined = CallManager::instance().joinConference(accountIdCall1, callIdA, accountIdCall2, callIdB);
...
CallModel::computeFallbackConversation(bool currentIsCoreDialog,
...
{
    if (!currentIsCoreDialog) {
        return std::nullopt;
    }

    if (remoteParticipantUris.isEmpty()) {
        return std::nullopt;
    }

    QString fallbackUid;

    for (const auto& uri : remoteParticipantUris) {
        auto it = peerUriToConversationUid.constFind(uri);
        if (it == peerUriToConversationUid.constEnd() || it.value().isEmpty()) {
            continue;
        }

        // If any remaining participant maps to the current conversation then no switch needed
        if (it.value() == currentConversationUid) {
            return std::nullopt;
        }

        // Otherwise remember the first available conversation candidate
        if (fallbackUid.isEmpty()) {
            fallbackUid = it.value();
        }
    }
...
CallModelPimpl::callAdvancedInformation()
{
    ...
    for (const auto& callId : callList) {
        MapStringString mapStringDetailsList = CallManager::instance().getCallDetails(linked.owner.id, callId);
...
CallModelPimpl::getFallbackConversationForConference(const QString& confId)
{
    ...
    // Collect remote participants (excluding self, avoid duplicates)
    // participantsModel is populated from confInfos, which is entirely handled by the videomixer
    // and based on active RTP streams. It can be unreliable during transitions (e.g. camera activation)
    // where streams are temporarily missing.
    // Therefore a stable API from the daemon based on subcalls would be nice
    ...
    for (const auto& uri : remoteParticipantsUris) {
        ...
        if (conv) {
            peerUriToConversationUid.insert(uri, conv->get().uid);
        }
    }

    auto fallbackUid = CallModel::computeFallbackConversation(currentConversation->get().isCoreDialog(),
                                                              currentConversation->get().uid,
                                                              remoteParticipantsUris,
                                                              peerUriToConversationUid);
    if (!fallbackUid) {
        return std::nullopt;
    }

    auto fallbackConv = linked.owner.conversationModel->getConversationForUid(fallbackUid.value());
    if (!fallbackConv) {
        qWarning() << "Fallback conversation UID chosen but not found:" << fallbackUid.value();
        return std::nullopt;
    }

    qWarning() << "Fallback conversation found with UID:" << fallbackUid.value();
    return fallbackConv;
}

void
CallModelPimpl::applyFallbackConversationSwitch(const QString& confId,
                                                OptRef<conversation::Info> currentConversation,
                                                OptRef<conversation::Info> fallbackConversation,
                                                const QString& newCallId,
                                                const QString& newConfId)
...
CallModelPimpl::initCallFromDaemon()
{
    ...
    for (const auto& callId : callList) {
        ...
        callInfo->type = call::Type::DIALOG;
        VectorMapStringString infos = CallManager::instance().getConferenceInfos(linked.owner.id, callId);
...
void
CallModel::setActiveStream(
    const QString& confId, const QString& accountUri, const QString& deviceId, const QString& streamId, bool state)
{
...
CallModel::isModerator(const QString& confId, const QString& uri)
{
    ...
    auto isModerator = uriToCheck == ownerUri ? call->second->type == lrc::api::call::Type::CONFERENCE : false;
...
void
CallModel::raiseHand(const QString& confId, const QString& accountUri, const QString& deviceId, bool state)
{
                                         ...
                                         const VectorMapStringString& mediaList)
{
    ...
    CallManager::instance().answerMediaChangeRequest(linked.owner.id,
                                                     callId,
                                                     QVector<MapStringString>::fromList(answerMedia));
}
...
CallModelPimpl::slotCallStateChanged(const QString& accountId, const QString& callId, const QString& state, int code)
{
    ...
    if (call->status == call::Status::ENDED) {
        ...
        if (previousStatus == call::Status::INCOMING_RINGING || previousStatus == call::Status::OUTGOING_RINGING) {
        ...
        for (int i = 0; i < pendingConferencees_.size(); ++i) {
            if (pendingConferencees_.at(i).callId == callId) {
                linked.joinCalls(pendingConferencees_.at(i).callIdToJoin, pendingConferencees_.at(i).callId);
                break;
            }
        }
...
void
CallModelPimpl::slotMediaNegotiationStatus(const QString& callId, const QString&, const VectorMapStringString& mediaList)
{
    if (!linked.hasCall(callId)) {
        return;
    }
                                       ...
                                       const QString& payload)
{
    ...
    if (it != vcardsChunks.end()) {
        ...
        if (not std::any_of(vcardsChunks[from].begin(), vcardsChunks[from].end(), [](const auto& s) {
                return s.isEmpty();
            })) {
...
void
CallModelPimpl::slotVoiceMailNotify(const QString& accountId, int newCount, int oldCount, int urgentCount)
{
...
CallModelPimpl::slotOnConferenceInfosUpdated(const QString& confId, const VectorMapStringString& infos)
{
    ...
    // If the conference only has 1 subcall left and getting destroyed, it will be handled by slotConferenceRemoved
    // Otherwise, if the callist size decreased, we need to check if we need to switch conversation
    bool participantHasLeft = previousCallListSize > callListSize;
    bool conferenceIsOngoing = callListSize > 1;
...
CallModelPimpl::slotConferenceCreated(const QString& accountId, const QString& conversationId, const QString& confId)
{
    ...
    VectorMapStringString infos = CallManager::instance().getConferenceInfos(linked.owner.id, confId);
...
void
CallModelPimpl::slotConferenceChanged(const QString& accountId, const QString& confId, const QString&)
{
    if (accountId != linked.owner.id)
        return;
...
CallModelPimpl::slotConferenceRemoved(const QString& accountId, const QString& confId)
{
    ...
    // If a fallback conversation was returned, we should switch to it
    if (fallbackConversation) {
        applyFallbackConversationSwitch(confId, currentConversation, fallbackConversation, remainingCallId, QString {});
