...
CallModelPimpl::slotConferenceRemoved(const QString& accountId, const QString& confId)
{
    ...
    if (participantsIt != participantsModel.end()) {
        ...
        qDebug() << "Found" << participants.size() << "participants in conference:" << confId;
        for (const auto& participant : participants) {
            qDebug() << "Participant URI:" << participant.uri;
            if (participant.uri != linked.owner.profileInfo.uri) {
                ...
                try {
                    ...
                    qDebug() << "Found remaining peerUri:" << remainingPeerUri << "with callId:" << remainingCallId;
    ...
    if (!currentConvUid.isEmpty() && !fallbackConvUid.isEmpty() && currentConvUid == fallbackConvUid) {
        ...
        qDebug() << "No fallback conversation available to switch to. Staying on:" << currentConvUid;
    } else {
