...
void
CurrentDevice::setVideoDefaultDevice(int index)
{
    auto deviceId = videoDeviceFilterModel_->data(videoDeviceFilterModel_->index(index, 0),
                                                  VideoInputDeviceModel::DeviceId);
    lrcInstance_->avModel().setDefaultDevice(deviceId.toString());
}
