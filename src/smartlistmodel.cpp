...
SmartListModel::rowCount(const QModelIndex& parent) const
{
    if (!parent.isValid()) {
        ...
        if (listModelType_ == Type::TRANSFER) {
            ...
            qDebug() << "@@@@row count3 " << convModel->getFilteredConversations(filterType).size();
            ...
            qDebug() << "@@@@row count2 " << rowCount;
            return rowCount;
        }
        qDebug() << "@@@@row count1 " << conversations_.size();
...
SmartListModel::data(const QModelIndex& index, int role) const
{
    ...
    try {
        ...
        if (listModelType_ == Type::TRANSFER) {
            ...
            const auto& item = LRCInstance::getConversationFromConvUid(itemConvUid, itemAccountId);
