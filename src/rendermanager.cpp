...
FrameWrapper::FrameWrapper(AVModel& avModel, const QString& id)
    ...
    , pFrameCorrectFormat_ {av_frame_alloc(),
                            [](AVFrame* frame) {
                                av_frame_free(&frame);
                            }}
...

    {
        ...
        if (useOldPipline_) {
            ...
            if (currentFormat == targetFormat || currentFormat == AVPixelFormat::AV_PIX_FMT_YUV422P
                ...
                pFrameCorrectFormat_.reset(av_frame_alloc());
                ...
                avpicture_fill((AVPicture*) (pFrameCorrectFormat_.get()),
                ...
                sws_scale(imgConvertCtx_.get(),
                          ...
                          0,
                          avFrame->height,
                          pFrameCorrectFormat_->data,
                          pFrameCorrectFormat_->linesize);
                pFrameCorrectFormat_->height = avFrame->height;
                pFrameCorrectFormat_->width = avFrame->width;
                pFrameCorrectFormat_->format = targetFormat;
                av_frame_copy_props(pFrameCorrectFormat_.get(), avFrame.get());
                avFrame_.release();
                avFrame_ = std::move(pFrameCorrectFormat_);
            }
...
AVFrame*
FrameWrapper::transferToMainMemory(AVFrame* frame, int desiredFormat)
{
    ...
    frameFromHW->format = desiredFormat;
