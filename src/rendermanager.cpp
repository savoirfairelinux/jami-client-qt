...
FrameWrapper::renderAVFrame()
{
    ...
    if (not avframe or avframe->buf[0] == nullptr or avframe->buf[0]->size <= 0) {
        qWarning() << QString("Invalid avframe");
        return;
    }

    if (width != avframe->width || height != avframe->height) {
        qWarning() << QString("Size mismatch frame %1x%2 vs renderer %3x%4. Ignore")
                          .arg(width)
                          .arg(height)
                          .arg(avframe->width)
                          .arg(avframe->height);
    ...
    size = avframe->buf[0]->size;
    QImage::Format imageFormat = QImage::Format_ARGB32_Premultiplied;

    qsizetype stride = avframe->linesize[0];
