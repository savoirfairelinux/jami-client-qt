...
VideoProvider::copyUnaligned(QVideoFrame* dst, const video::Frame& src)
{
    ...
    assert(dst->pixelFormat() == QVideoFrameFormat::Format_RGBA8888 or
        dst->pixelFormat() == QVideoFrameFormat::Format_BGRA8888);
    ...
    if (dst->width() * dst->height() * BYTES_PER_PIXEL != src.size) {
        qCritical() << "Size mismatch. Actual " << src.size 
                 << " Expected " << dst->width() * dst->height() * BYTES_PER_PIXEL;
        return;
    }
