...
DBusErrorHandler::errorCallback()
{
    ...
    emit LRCInstance::instance().showDaemonReconnectPopup(true);

    QTimer::singleShot(2500, []() {
        if ((!lrc::api::Lrc::isConnected()) || (!lrc::api::Lrc::dbusIsValid())) {
            qDebug() << "Could not reconnect to the daemon";
            emit LRCInstance::instance().daemonReconnectFailed();
        } else {
...
{
    handlingError = false;
    emit LRCInstance::instance().showDaemonReconnectPopup(false);
}
