...
PluginHandlerItemListModel::PluginHandlerItemListModel(QObject* parent, const QString& accountId, const QString& peerId)
...
PluginHandlerItemListModel::rowCount(const QModelIndex& parent) const
{
    if (!parent.isValid()) {
        ...
        if (isMediaHandler_)
            ...
            return LRCInstance::pluginModel().listChatHandlers().size();
    }
    /*
...
PluginHandlerItemListModel::data(const QModelIndex& index, int role) const
{
    ...
    if (isMediaHandler_) {
        ...
        for (const auto& mediaHandler : LRCInstance::pluginModel().getCallMediaHandlerStatus(callId_))
        ...
        auto chathandlerList = LRCInstance::pluginModel().listChatHandlers();
        ...
        for (const auto& chatHandler : LRCInstance::pluginModel().getChatHandlerStatus(accountId_, peerId_))
