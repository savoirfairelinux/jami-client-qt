   ...
   /*  QString cacheFile = QStandardPaths::writableLocation(QStandardPaths::CacheLocation)
    ...
    if (file.exists() && file.open(QIODevice::ReadOnly)) {
        ...
        if (!doc.isNull() && doc.isObject()) {
            ...
            qWarning() << "Loaded available dictionaries from cache";
            return cachedAvailableDictionaries_;
        }
    }

    // If both static data and cache fail, populate from network
    //populateAvailableDictionaries(); */
    return cachedAvailableDictionaries_;
}
...
SpellCheckDictionaryManager::populateAvailableDictionaries()
{
    ...
    /* spellCheckFileDownloader->sendGetRequest(dictionariesURL, [this](const QByteArray& data) {
                ...
                if (match.hasMatch()) {
                    ...
                    if (!foundDictionaries.contains(locale)) {
                        ...
                        if (!nativeName.isEmpty()) {
                            ...
                            dictInfo["path"] = path.left(path.length() - 4);
        ...
        QString cacheFile = cacheDir + "/available_dictionaries.json";
        QFile file(cacheFile);

        if (file.open(QIODevice::WriteOnly)) {
            QJsonDocument saveDoc(cachedAvailableDictionaries_);
            file.write(saveDoc.toJson());
            file.close();
            qWarning() << "Available dictionaries cache saved to:" << cacheFile;
        } else {
            qWarning() << "Failed to save available dictionaries cache";
        }
    }); */
}
