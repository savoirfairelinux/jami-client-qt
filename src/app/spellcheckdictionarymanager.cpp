...
SpellCheckDictionaryManager::populateAvailableDictionaries()
{
    ...
    if (jsonString.isEmpty()) {
        SP_ERR << "Could not load available dictionaries list";
        return;
    }
    ...
    if (!doc.isNull() && doc.isObject()) {
        ...
        SP_ERR << "Invalid JSON format in available dictionaries file";
    }
}
...
SpellCheckDictionaryManager::getBestDictionary(QString locale)
{
    ...
    if (locale == "NONE") {
        ...
        SP_DBG << "Locale set to user interface language:" << locale;
    }
...
SpellCheckDictionaryManager::downloadDictionary(QString languagePath)
{
    if (getDictionaryUrl().isEmpty()) {
        SP_ERR << "Dictionary " << languagePath << " cannot be downloaded : No dictionary URL set";
        return;
    }
