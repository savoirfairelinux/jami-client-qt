...
//
// A lone URL is not text of that kind: it is what a browser offers alongside an
// image copied from a page, the address of that very image. Treating it as text
// made copying a picture from a browser paste its address instead of itself.
static bool
textWins(const QMimeData* mime)
{
    if (!mime->hasText())
        return false;
...
CollabRichBinding::imageFromMimeData(const QMimeData* mime)
{
    ...
    if (carriesEncodedImage(mime) || (mime->hasImage() && !textWins(mime))) {
        for (const char* type : IMAGE_MIME_TYPES) {
...
CollabRichBinding::mimeCarriesImage(const QMimeData* mime)
{
    ...
    if (mime->hasImage() && !textWins(mime)) {
        const QImage image = qvariant_cast<QImage>(mime->imageData());
