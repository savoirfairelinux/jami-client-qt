...
QtObject {
    ...
    // Create a view from a path only if it doesn't already exist. This is used by the view coordinator
    // to create views that are not self-destructing (main views) and only exist once per instance of the app.
    ...
    // Create a new view. Useful when we want to create multiple views that are self-destructing (dialogs).
    function createUniqueView(path, parent = null, cb = null, props = {}) {
        const component = Qt.createComponent(Qt.resolvedUrl(path));
        return createViewFromComponent(component, getViewName(path), parent, cb, props);
    }

    // Create a new view from a component. If a view with the same path already exists, it is returned instead.
    function createViewFromComponent(component, viewName, parent = null, cb = null, props = {}) {
    ...
    // Finds a unique view name for a given path by appending a number to the base name.
    // For example, if a view named "MyView" already exists, the next view will be named "MyView_1".
