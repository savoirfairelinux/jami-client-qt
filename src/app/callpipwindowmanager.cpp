...
CallPipWindowManager::~CallPipWindowManager()
{
    // Explicitly disconnect all callModel signals so that no daemon callback
    // can fire into this object after it has been destroyed. Qt disconnects
    // signals from destroyed receivers automatically, but only after ~QObject()
    // has run; a direct-connection signal arriving from the daemon thread
    // between the start of destruction and that point is undefined behaviour.
    disconnectCallModel();
}
