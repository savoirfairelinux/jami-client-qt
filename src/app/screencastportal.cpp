...
#define REQUEST_PATH "/org/freedesktop/portal/desktop/request/%s/pipewiregrab%u"
#define SESSION_PATH "/org/freedesktop/portal/desktop/session/%s/pipewiregrab%u"
         ...
         const char* sender_name)
{
    (*token_count)++;
    if (out_token)
        *out_token = g_strdup_printf("pipewiregrab%u", *token_count);
    if (out_path)
        *out_path = g_strdup_printf(path_format, sender_name, *token_count);
}
                                                  ...
                                                  gpointer user_data)
{
    ...
    g_variant_iter_loop(&iter, "(u@a{sv})", &portal->pipewire_node, &stream_properties);
...
ScreenCastPortal::start()
{
    ...
    new_path(&request_path, &request_token, &request_token_count, REQUEST_PATH, sender_name);
...
ScreenCastPortal::createSession()
{
    ...
    new_path(&request_path, &request_token, &request_token_count, REQUEST_PATH, sender_name);
    new_path(NULL, &session_token, &session_token_count, SESSION_PATH, NULL);
