...
public:
    ...
    {
#ifdef BUILD_TESTING
        return !pipConvId_.isEmpty() && (!window_.isNull() || testPipActive_);
#else
...
#ifdef BUILD_TESTING
    // Test-only: directly activate PiP state without creating a QQuickWindow.
    // Allows testing state-management logic in environments where QWindowKit is
    // unavailable (e.g. offscreen CI test runs).
