...
//        }
    ...
    function currentAccountIsCalling() {
        return UtilsAdapter.hasCall(CurrentAccount.id)
    }
    ...
    Connections {
        ...
        function onHasCallChanged() {
            if (CurrentConversation.hasCall) {
                ...
                viewCoordinator.dismiss("CallStackView")
            }
        }
    }
...
//                        CurrentAccount.id)
//            LRCInstance.selectConversation(callConv, CurrentAccount.id)
//            CallAdapter.updateCall(callConv, CurrentAccount.id)
//        } else {
//            showWelcomeView()
//        }
    }
    ...
    function setMainView(convId) {
        print("&&&&&&&& setMainView", convId)
        if (convId === '') {
            return
        }
        var callViewShouldShow = false
        if (CurrentCall.exists) {
            if (CurrentCall.isOutgoing) {
                // Outgoing
                callViewShouldShow = Call.status !== Call.Status.ENDED
            } else {
                // Incoming
                callViewShouldShow = (CurrentCall.status === Call.Status.IN_PROGRESS ||
                                      CurrentCall.status === Call.Status.PAUSED ||
                                      CurrentCall.status === Call.Status.INCOMING_RINGING)
            }
        }
        if (callViewShouldShow) {
            // callStackView.setLinkedWebview(chatView)
            pushCallStackView()
//            var callStackView = viewCoordinator.currentItem
//            if (CurrentCall.status === Call.Status.IN_PROGRESS ||
//                    CurrentCall.status === Call.Status.PAUSED) {
//                CallAdapter.updateCall(convId, LRCInstance.currentAccountId)
//                callStackView.showOngoingCallPage()
//            } else {
//                callStackView.showInitialCallPage(CurrentCall.callState, CurrentCall.isAudioOnly)
//            }
    ...
    Connections {
        ...
        function onIdChanged() {
            print("****************2", LRCInstance.selectedConvUid, CurrentConversation.id)
            mainView.setMainView(CurrentConversation.id)
        }
    }
...
//            print("****************1", LRCInstance.selectedConvUid, CurrentConversation.id)
