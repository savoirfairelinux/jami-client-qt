    ...
    TextArea.flickable: TextArea {
        ...
        function updateUnderlineText() {
            ...
            for (var i = 0; i < words.length; i++) {
                var word = words[i];
                var wordIndex = textArea.text.indexOf(word, cursorIndex); // Find the position of the word in the text
                ...
                cursorIndex = wordIndex + word.length; // Update cursor index
                textArea.deselect();
            }
        ...
        function clearUnderlines() {
            ...
            while (underlineList.length > 0) {
                var underlineObject = underlineList[underlineList.length - 1]; // Get the previous item
                underlineList.pop(); // Remove the last item
                underlineObject.destroy(); // Destroy the removed item
            }
        }
    }
}
