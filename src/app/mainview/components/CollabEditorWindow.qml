...
Window {
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            // While previewing, the editor holds a past version: its cursor
            // positions mean nothing in the live document, and broadcasting them
            // would move this device's caret around on everyone else's screen.
            if (root.applyingRemote || root.previewing)
                return;
