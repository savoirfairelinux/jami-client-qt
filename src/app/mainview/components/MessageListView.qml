...
JamiListView {
    ...
    function computeTimestampVisibility(item1, item1Index,item2,item2Index) {
        ...
        if(item1 && item2){
            if(item1Index < item2Index){
                item1.showTime = item1.timestamp - item2.timestamp  > 120
                item1.showDay = item1.formattedDay !== item2.formattedDay
                //console.warn('qq',item1Index,item1.showTime,item1.timestamp, item2.timestamp)
            }else{
                item2.showTime = item2.timestamp - item1.timestamp  > 120
    ...
    function newCompute(item,itemIndex) {
        if (!root ) return
        ...
        //Middle insertion
        if(pItem && nItem){
            //Check if the beahviour really is like a bottom insertion
        ...
        if(itemIndex === 0){
            ...
            if (! computeTimestampVisibility(item,itemIndex,nItem,nItemIndex)){
                Qt.callLater(newCompute,item,itemIndex)
            }
        }
        //top element
