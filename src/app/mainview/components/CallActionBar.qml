        ...
        Action {
            ...
            onTriggered: {
                ...
                if (CurrentCall.isSharing) {
                    // When sharing screen/window (not file), show audio toggle option on all systems.
                    // Daemon-side filtering ensures audio is only added for supported configurations:
                    // - 1:1 calls only (not in conferences)
                    // - Platform-specific restrictions apply at daemon level
            ...
            function accept(index) {
                ...
                if (CurrentCall.isSharing) {
                    if (item.Name === JamiStrings.shareAudio) {
                        if (AvAdapter.muteScreenshareAudio === true) {
                            // If audio is currently muted, enable it
                            AvAdapter.toggleScreenshareAudio(false);
                            return;
                        } else {
                            // If audio is currently shared, mute it
                            AvAdapter.toggleScreenshareAudio(true);
                            return;
                        }
                    }
                } else {
