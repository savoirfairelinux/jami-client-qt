...
isCompatibleQtRuntimeVersion(const QString& runtimeVersion, const QString& buildVersion)
{
    ...
    if (runtime.segmentCount() < requiredSegmentCount || build.segmentCount() < requiredSegmentCount) {
        return false;
    }
    ...
    : QApplication(argc, argv)
{
    ...
    if (!isCompatibleQtRuntimeVersion(qtVersion, QT_VERSION_STR)) {
        ...
        C_ERR << "Qt runtime version" << qtVersion << "is incompatible with build version" << QT_VERSION_STR;
    } else if (qtVersion != QLatin1String(QT_VERSION_STR)) {
        C_WARN << "Qt runtime patch version" << qtVersion << "differs from build version" << QT_VERSION_STR;
    }
