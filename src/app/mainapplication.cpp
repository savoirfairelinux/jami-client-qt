...
#ifdef QT_DEBUG
    // In debug mode, add an option to test a specific QML component via its local URI.
    // e.g. --test qrc:/MyComponent.qml
    QCommandLineOption qmlTestOption("test",
                                     "Test a specific QML component via its local URI.",
                                     "uri");
    parser_.addOption(qmlTestOption);
#endif
...
findResource(const QString& targetBasename, const QString& basePath = ":/")
{
    ...
    Q_FOREACH (const QString& entry, entries) {
        ...
        } else if (fileInfo.isFile() && fileInfo.fileName() == targetBasename) {
...
MainApplication::initQmlLayer()
{
    ...
    if (parser_.isSet("test")) {
        ...
        const auto targetTestComponent = findResource(
            "AccountComboBox.qml"); // parser_.value("test"));
