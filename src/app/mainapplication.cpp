...
void
MainApplication::fixMacOSRoundedCorners(QWindow* window)
{
#ifdef Q_OS_MACOS
    if (window) {
        macutils::fixMacOSRoundedCorners(window);
    }
#endif
}

void
MainApplication::setupWindowDelegate(QWindow* window)
{
#ifdef Q_OS_MACOS
    if (window) {
        macutils::setupWindowDelegate(window);
    }
#endif
}
