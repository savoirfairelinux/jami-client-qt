...
BaseModalDialog {
    ...
    property string errorTitle: JamiStrings.callFailed
    ...
    // Function to set error details based on code
    function showError(errorCode) {
        switch(errorCode) {
            case 415: // Unsupported Media Type
                errorReason = JamiStrings.mismatchedCodecTitle;
                errorDescription = JamiStrings.mismatchedCodecDescription;
                break;
            default:
                errorReason = qsTr("Error (%1)").arg(errorCode);
                errorDescription = qsTr("An error occurred during the call. Please try again.");
                break;
        }
        open();
    }
