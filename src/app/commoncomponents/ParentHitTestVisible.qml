...
Item {
    ...
    // Note: this is experimental. TBD if this is the best way to do it.
    // This technique makes it clear and simple to implement, but may have
    // side effects beyond just adding a dummy item component.
    // Alternatives:
    // - Use object names with a prefix, and iterate through all children of
    //   the parent to find the ones with the prefix and set them to be hit
    //   test visible.
    // - Use a property on the parent to specify which children should be hit
    //   test visible. Also iterate through all children of the parent.
