...
function resetPoints () {
    ...
    for (var i = 0; i < layerArray.length; i++  ){
        if(layerArray[i].layer_type === 'position') {
...
function printIcon (coordos, authorI) {
    ...
    iconLayer.layer_type = 'position';
    //add the icon to the layer
    map.addLayer(iconLayer);
}

function setAvatarList (peerId, image) {
    console.log('avatarList set 1')
    curDict = {}
    curDict["author"] = peerId;
    curDict["image"] = image;
    dict.push(curDict);
    console.log('avatarList set 2')
}

function setPosition (coordos) {
    console.log('posoitionStart')
    ...
    heatMapLayer.layer_type = 'position';
    // add to the map 
