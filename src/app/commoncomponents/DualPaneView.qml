...
BaseView {
    ...
    Component.onCompleted: {
        ...
        rightPaneItem.parent = Qt.binding(() => isSinglePane ? sv1 : sv2)
    }

    property bool isSinglePane
    property Item activeParent: isSinglePane ? singlePaneContainer : splitViewQCI
    property real previousLeftPaneWidth: sv1.width
    onWidthChanged: {
        if (!isSinglePane) previousLeftPaneWidth = sv1.width
        isSinglePane = splitView.width < minimumRightPaneWidth + previousLeftPaneWidth
    }
    ...
    SplitView {
        ...
        onVisibleChanged: visible ? restoreSplitViewState() : saveSplitViewState()
