...
Item {
    ...
    function getIsDonationBannerVisible() {
        ...
        const isDonationVisible = UtilsAdapter.getAppValue(Settings.Key.IsDonationVisible);
        const endDonationDate = Date.parse(UtilsAdapter.getAppValue(Settings.Key.Donation2023EndDate));
        const donationVisibleDate = Date.parse(UtilsAdapter.getAppValue(Settings.Key.Donation2023VisibleDate));
        const now = new Date();
        return isDonationVisible && now < endDonationDate && now >= donationVisibleDate;
    }
}
