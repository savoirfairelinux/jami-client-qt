...
Item {
    ...
    onCurrentConvIdChanged: {
        if (attachedConvId  && !root.isUnpin) {
            closeMapPosition()
        }
    }

    function closeMapPosition() {
        PositionManager.setMapInactive(PositionManager.convertKeytoString(attachedAccountId, attachedConvId))
    ...
    Window {
        ...
        visible: false
        title: JamiStrings.conversation + " - " + PositionManager.getmapConvTitle(attachedAccountId, attachedConvId)
            ...
            WebEngineView {
                ...
                Connections {
                    ...
                    function onPositionShareAdded(shareInfo) {
                        if(webView.isLoaded) {
                            if (PositionManager.isUriInConversation(shareInfo.author, attachedAccountId, attachedConvId)) {
                    ...
                    function onPositionShareRemoved(author) {
                        if(webView.isLoaded) {
                            if (PositionManager.isUriInConversation(author, attachedAccountId, attachedConvId)) {
                        ...
                        onClicked: {
                            onClicked: {
                                if (!root.isUnpin) {
                                    ...
                                    root.isUnpin = true
                                    windowUnpin.show()
                                    PositionManager.setMapActive(true, PositionManager.convertKeytoString(CurrentAccount.id, CurrentConversation.id))
                                } else {
                                    mapPopup.state = "pin"
