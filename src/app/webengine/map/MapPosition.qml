...
Item {
    ...
    onCurrentAccountIdChanged: {
        if (attachedAccountId  && !root.isUnpin) {
            closeMapPosition()
            console.warn("close map account change")
        }
    }
    ...
    Window {
        ...
        visible: false
        title: JamiStrings.conversation + " - " + PositionManager.getmapTitle(attachedAccountId)
                        ...
                        onClicked: {
                            onClicked: {
                                if (!root.isUnpin) {
                                    ...
                                    root.isUnpin = true
                                    windowUnpin.show()
                                    PositionManager.unPinMap(CurrentAccount.id)
                                } else {
                                    mapPopup.state = "pin"
                                    PositionManager.pinMap(CurrentAccount.id)
                                    windowUnpin.close()
                                }
                            }
                        }
