...
Item {
    ...
    property real maxWidth: mapLoader.parent.width
    property real maxHeight: mapLoader.parent.height
    ...
    Window {
        ...
        visible: false
        title: JamiStrings.conversation + " - " + PositionManager.getmapConvTitle()
        ...
        onClosing: {
            if (PositionManager.getsharedConvId() !== CurrentConversation.id)
                PositionManager.setMapActive(false)
    ...
    Item {
        ...
        Rectangle {
            ...
            onIsFullScreenChanged: {
                console.warn("isFullScreen: ", isFullScreen)
            }
            ...
            states: [ State {
                ...
                State {
                    name: "pin"
                    ParentChange { target: mapPopup; parent: parentPin; x:xPos; y:yPos }
                }
            ]
                        ...
                        onClicked: {
                            onClicked: {
                                if (!root.isUnpin) {
                                    ...
                                    root.isUnpin = true
                                    windowUnpin.show()
                                    PositionManager.lockMapConv(CurrentConversation.id, CurrentAccount.id)
                                } else {
                                    mapPopup.state = "pin"
                                    windowUnpin.close()
                                }
