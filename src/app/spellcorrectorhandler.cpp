...
SpellCorrectorHandler::availableDictionaries(){
    ...
    //We fetch in a list all of the folders present at the mirror URL
    auto dictionariesDir = QDir(dictionariesURL);
    qWarning() << "Dictionaries URL:" << dictionariesURL;
    auto folders = dictionariesDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
    for (const auto& folder : folders) {
        qWarning() << "Processing folder:" << folder;
        auto subDir = dictionariesDir.absoluteFilePath(folder);
        //auto dicFiles = subDir.entryList(QStringList() << "*.dic", QDir::Files);
        //qWarning() << "Dictionary files found in folder" << folder << ":" << dicFiles;
    }
    QVariantMap result;
