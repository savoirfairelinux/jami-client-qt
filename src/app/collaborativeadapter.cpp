                           ...
                           const QString& insert)
{
    ...
    if (!replica)
        return;
    // QML hands these as plain ints. A negative one would wrap around to a huge
    // uint32_t and be clamped to the end of the document, silently editing the
    // wrong place, so nonsense is rejected outright instead.
    if (index < 0 || deleteLen < 0) {
        qWarning() << "Ignoring a collaborative edit with a negative index or length";
        return;
    }
...
{
    QVariantList result;
    // 0 means "no limit" all the way down to the daemon, and the parameter is
    // unsigned there: a negative int from QML would ask for the entire history of
