...
class AvAdapter final : public QmlAdapterBase
{
    ...
    QML_PROPERTY(bool, shareStreamAudio)
...
protected:
    ...
    Q_INVOKABLE void shareEntireScreen(int screenNumber, bool withDesktopAudio = false);
    ...
    Q_INVOKABLE void shareEntireScreenWayland(bool withDesktopAudio = false);
#endif

    // Share the all screens connected.
    Q_INVOKABLE void shareAllScreens(bool withDesktopAudio = false);
    ...
    Q_INVOKABLE void shareScreenArea(unsigned x, unsigned y, unsigned width, unsigned height, bool withDesktopAudio = false);

    // Select window to display (all platforms except Wayland).
    Q_INVOKABLE void shareWindow(const QString& windowProcessId,
                                 const QString& windowId,
                                 const int fps = -1,
                                 bool withWindowAudio = false);
    ...
    Q_INVOKABLE void shareWindowWayland(bool withWindowAudio = false);
#endif
    ...
    Q_INVOKABLE void toggleShareAudio(bool enable);
...
private:
    ...
    void shareWayland(bool entireScreen, bool withAudio = false);
#endif
