...
clearCompletedReports(crashpad::CrashReportDatabase* database)
{
    ...
    if (reports.empty()) {
        C_WARN << "No completed reports to delete";
        return;
    }
    ...
    : CrashReportClient(settingsManager, parent)
{
    try {
        ...
        // we will just not start the handler here in that case.
        if (!crashedLastRun_) {
            startHandler();
        }
...
CrashPadClient::uploadLastReport()
{
    ...
    QThreadPool::globalInstance()->start([this]() {
        ...
        while (attempts++ < maxAttempts) {
            ...
            C_INFO << "Checking for pending reports";
            if (database->GetPendingReports(&pendingReports) == CrashReportDatabase::kNoError
                && pendingReports.empty()) {
                C_INFO << "Report uploaded successfully";
