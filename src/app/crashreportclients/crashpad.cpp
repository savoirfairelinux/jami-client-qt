...
#ifndef Q_OS_WIN
static std::optional<crashpad::CrashReportDatabase::Report>
getReportForUuid(crashpad::CrashReportDatabase* database, const crashpad::UUID& uuid)
{
    using OperationStatus = crashpad::CrashReportDatabase::OperationStatus;

    auto findReport = [&database, &uuid](auto getReportsFunc, const char* errorMsg)
        -> std::optional<crashpad::CrashReportDatabase::Report> {
        std::vector<crashpad::CrashReportDatabase::Report> reports;
        auto status = (database->*getReportsFunc)(&reports);
        if (OperationStatus::kNoError != status) {
            C_WARN << errorMsg;
            return std::nullopt;
        }
        ...
        for (const auto& report : reports) {
            if (report.uuid == uuid) {
                return std::make_optional(report);
            }
        }
        return std::nullopt;
    };
    ...
    if (auto report = findReport(&crashpad::CrashReportDatabase::GetCompletedReports,
                                 "Crashpad database GetCompletedReports failed")) {
        return report;
    }

    // Check pending reports
    return findReport(&crashpad::CrashReportDatabase::GetPendingReports,
                      "Crashpad database GetPendingReports failed");
}
#endif
...
CrashPadClient::uploadLastReport()
{
    ...
    // On Windows, we immediately clear completed reports without waiting for upload confirmation.
    // This is because Crashpad on Windows doesn't reliably set the "uploaded" flag in its database,
    // making it impossible to verify upload status.
#ifdef Q_OS_WIN
    // Let's wait for the report to be uploaded then clear all reports on a
    // separate thread to avoid blocking the crashpad client thread.
...
#else
    // On non-Windows platforms, we wait for upload confirmation before removing the report

    // Try up to 10 times with 100ms between attempts (total wait: 1 second)
    const int maxAttempts = 10;
    const auto timeout = std::chrono::milliseconds(100);

    for (int attempts = 0; attempts < maxAttempts; ++attempts) {
        // Query the database to check if this specific crash report has been uploaded
        C_INFO << "Checking if report has been uploaded";
        auto reportAwaitingUpload = getReportForUuid(database.get(), report.uuid);

        // If we found the report and its uploaded flag is set to true
        if (reportAwaitingUpload.has_value() && reportAwaitingUpload->uploaded) {
            C_INFO << "Report uploaded successfully, removing it from the database";

            // Remove the report from the database since it's been uploaded
            status = database->DeleteReport(reportAwaitingUpload->uuid);
            if (status != CrashReportDatabase::kNoError) {
                C_WARN << "Failed to delete report, status: " << status;
            }
            return;
        }
    ...
    // If we get here, we failed to confirm the upload after all attempts
    C_WARN << "Failed to upload report";
#endif
}
