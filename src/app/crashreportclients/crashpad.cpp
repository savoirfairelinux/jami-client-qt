...
CrashPadClient::uploadLastReport()
{
    ...
    QThreadPool::globalInstance()->start([this, dbPath = dbPath_, safeThis = QPointer(this)]() {
        auto database = crashpad::CrashReportDatabase::Initialize(base::FilePath(dbPath));
        if (database == nullptr) {
            C_WARN << "Crashpad database initialization failed";
            return;
        ...
        for (int attempts = 0; attempts < maxAttempts; ++attempts) {
            ...
            C_DBG << "Checking for pending reports";
            std::vector<crashpad::CrashReportDatabase::Report> pendingReports;
            if (database->GetPendingReports(&pendingReports)
                    == crashpad::CrashReportDatabase::kNoError
                && pendingReports.empty()) {
                C_INFO << "Report uploaded successfully, clearing completed reports";
