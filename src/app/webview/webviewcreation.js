...
/**
 * @brief the QML component that webviews are created from (actually a WebEngineView)
 */
const webViewComponent = Qt.createComponent("./WebViewCustom.qml");
 ...
 * @param attachReason the reason why this web view was opened (whenever it does get opened)
 * @returns the newly-created web view object
 */

// TODO: accept a signal to hook up to onLoadingChanged with WebEngineView.LoadSucceededStatus
//       to allow the parent to know when the webview is ready
function createWebView(parent, pluginId, accountId, attachReason) {
    let webViewObject = null;
    if (webViewComponent.status == Component.Ready) {
        ...
        if (webViewObject == null) {
            console.log("error creating webview object");
        } else {
            console.log("webview object created");
        }
    } else if (webViewComponent.status == Component.Error) {
        console.log("error loading webview component: ", webViewComponent.errorString());
    }
