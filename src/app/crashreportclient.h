...
{
    Q_OBJECT
    QML_PROPERTY(QVariantMap, annotations)

public:
    CrashReportClient(AppSettingsManager* settingsManager, QObject* parent = nullptr);
    ~CrashReportClient() = default;

    void startHandler();

    Q_INVOKABLE void setHandlerSettings();
    Q_INVOKABLE void setUploadsEnabled(bool enabled);
    Q_INVOKABLE void uploadLastReport();
    Q_INVOKABLE bool getCrashedLastRun();
