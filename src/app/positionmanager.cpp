...
PositionManager::loadPreviousLocations(QString& accountId, QString& convId)
{
    ...
    for (auto it = objectListSharingUris_.begin(); it != objectListSharingUris_.end(); it++) {
        if (true) {
            ...
            QVariantMap positionReceived = parseJsonPosition(strJson,
                                                             it.key().second,
                                                             it.key().first);
...
QVariantMap
PositionManager::parseJsonPosition(const QString& body,
                                         ...
                                         const QString& accountId)
{
    ...
    return !endSharing && (accountId == lrcInstance_->get_currentAccountId()) && mapAutoOpening_
           ...
           && (accountId == lrcInstance_->get_currentAccountId())
...
PositionManager::removePositionFromMemory(QPair<QString, QString> key, QVariantMap positionReceived)
{
    ...
    qWarning() << "remove position from memory active" << sharingUrisCount_
               << objectListSharingUris_.begin().key();
    ...
    if (!positionShareConvIds_.length()
        && ((sharingUrisCount_ == 1
            ...
            || sharingUrisCount_ == 0)) {
        qWarning() << "remove position from memory active";
        setMapInactive(key.first);
    }
}
                                    ...
                                    const QString& daemonId)
{
    ...
    // parse the position from json
    QVariantMap positionReceived = parseJsonPosition(body, peerId, accountId);
