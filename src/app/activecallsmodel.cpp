...
ActiveCallsModel::ActiveCallsModel(QObject* parent)
...
ActiveCallsModel::data(const QModelIndex& index, int role) const
{
    ...
    auto& call = static_cast<CurrentConversation*>(parent())->activeCalls().at(
        index.row());
    ...
    case Role::Ignored:
        return QVariant(ignored_.indexOf(
            MapStringString {{"id", call["id"]}, {"uri", call["uri"]}, {"device", call["device"]}}) != -1);
    }
    return QVariant();
}
...
ActiveCallsModel::ignore(const QString& id, const QString& uri, const QString& deviceId)
{
    ...
    auto index = static_cast<CurrentConversation*>(parent())->indexOfActiveCall(id, uri, deviceId);
    if (index != -1)
