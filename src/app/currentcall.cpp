    ...
    if (callInfo.status != lrc::api::call::Status::ENDED) {
        for (const auto& media : callInfo.mediaList) {
            if (media[MediaAttributeKey::MEDIA_TYPE] == Details::MEDIA_TYPE_VIDEO) {
                ...
                if (media[DRing::Media::MediaAttributeKey::SOURCE].startsWith(
...
CurrentCall::connectModel()
{
    try {
        ...
        qWarning() << "Can't connect current call signals" << e.what();
    }
}
...
{
    if (id_ != callId || lrcInstance_->get_currentAccountId() != accountId) {
        return;
    }
