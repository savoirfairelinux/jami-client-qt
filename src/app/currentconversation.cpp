...
CurrentConversation::updateData()
{
    ...
    try {
        ...
        if (accInfo.callModel && accInfo.callModel->hasCall(callId_)) {
        ...
        if (isCoreDialog_ && accInfo.contactModel)
            try {
...
CurrentConversation::onCallStatusChanged(const QString& accountId, const QString& callId, int)
{
    ...
    if (callModel && callModel->hasCall(callId_)) {
...
CurrentConversation::onShowIncomingCallView(const QString& accountId, const QString& convUid)
{
    ...
    auto convModel = lrcInstance_->getCurrentConversationModel();
    if (convModel) {
        auto optConv = convModel->getConversationForUid(convUid);
        if (!optConv)
            return;
        auto& convInfo = optConv->get();
