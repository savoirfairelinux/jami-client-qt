...
Window {
    ...
    // QtCreator may falsely report this with:
    // "Invalid property name". See QTCREATORBUG-13347
    onClosing: function(event) {
        event.accepted = false;

        if (UtilsAdapter.getAppValue(Settings.MinimizeOnClose)) {
            // Pop out any active call into PiP so it survives the minimize.
            // popOutFirstActiveCall() is a no-op if PiP is already active or there is no call.
