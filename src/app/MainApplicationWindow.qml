...
ApplicationWindow {
    ...
    onActiveChanged: {
        if (active && JamiQmlUtils.openFileDialogCount > 0) {
            // Find all views that contain a FileDialog
            for (const [path, view] of Object.entries(viewManager.views)) {
                // Check if view is a FileDialog
                if (view instanceof FileDialog) {
                    if (view.visible) {
                        view.parentWindow.requestActivate();
                        break;
                    }
                }
            }
