...
void
MessagesAdapter::onMessagesFoundProcessed(
    const QString& accountId, const QVector<QPair<QString, interaction::Info>>& messageInformation)
{
    if (lrcInstance_->get_currentAccountId() != accountId) {
        return;
    }

    bool isSearchInProgress = messageInformation.length();
    if (isSearchInProgress) {
        int index = -1;
        Q_FOREACH (const auto& msgPair, messageInformation) {
            index++;
            try {
                mediaInteractions_->insert(msgPair);
            } catch (...) {
