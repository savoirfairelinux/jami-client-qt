...
MessagesAdapter::getMediaInfo(const QString& msg)
{
    ...
    QRegularExpression vPattern("(video/)(avi|mov|webm|webp|rmvb)$",
                                QRegularExpression::CaseInsensitiveOption);
    QString type = vPattern.match(mime.name()).captured(2);
    if (!type.isEmpty()) {
        return {
            {"isVideo", true},
        ...
        QRegularExpression aPattern("(audio/)(ogg|flac|wav|mpeg|mp3)$",
                                    QRegularExpression::CaseInsensitiveOption);
        type = aPattern.match(mime.name()).captured(2);
        if (!type.isEmpty()) {
            return {
                {"isVideo", false},
