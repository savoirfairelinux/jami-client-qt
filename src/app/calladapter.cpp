...
CallAdapter::onCallStarted(const QString& callId)
{
    ...
    if (listener_->getPttState()){
        muteAudioToggle();
#ifdef HAVE_GLOBAL_PTT
        callId_ = lrcInstance_->getCallIdForConversationUid(lrcInstance_->get_selectedConvUid(),accountId_);
...
{
    if (callId_.isEmpty() || !lrcInstance_->getCurrentCallModel()->hasCall(callId_)) {
        qWarning() << "CallId is empty or call does not exist";
        return {};
    }
    auto* callModel = lrcInstance_->getCurrentCallModel();
    if (!callModel->hasCall(callId_)) {
        qWarning() << "CallModel does not have call";
        return {};
    }
    const auto callInfo = lrcInstance_->getCurrentCallModel()->getCall(callId_);
    auto mute = false;
