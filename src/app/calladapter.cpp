        ...
        for (const auto& media : callInfo->mediaList) {
            if (media["MEDIA_TYPE"] == "MEDIA_TYPE_VIDEO") {
                if (media["ENABLED"] == "true" && media["MUTED"] == "false") {
                    ...
                    isVideoMuted |= media["SOURCE_TYPE"] == "CAPTURE_DEVICE";
                    break;
                }
            }
        }
...
CallAdapter::muteAudioToggle()
{
    ...
    if (callModel->hasCall(callId)) {
        ...
        for (const auto& m: callInfo.mediaList) {
            if (m["SOURCE_TYPE"] == "CAPTURE_DEVICE" && m["MEDIA_TYPE"] == "MEDIA_TYPE_AUDIO") {
                mute = m["MUTED"] == "false";
            }
        }
...
CallAdapter::muteCameraToggle()
{
    ...
    if (callModel->hasCall(callId)) {
        ...
        // Note: here we do not use mute, because for video we can have several inputs, so if we are sharing
        // and showing the camera, we just want to remove the camera
        // TODO Enum
        if (mute)
