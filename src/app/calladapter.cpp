...
{
    qWarning() << "PiP muteAudioToggle: accountId=" << accountId << "convUid=" << convUid;
    const auto callId = lrcInstance_->getCallIdForConversationUid(convUid, accountId);
    qWarning() << "PiP muteAudioToggle: callId=" << callId;
    if (callId.isEmpty())
        return;
    try {
        auto& accInfo = lrcInstance_->getAccountInfo(accountId);
        auto* callModel = accInfo.callModel.get();
        if (!callModel->hasCall(callId)) {
            qWarning() << "PiP muteAudioToggle: callModel has no call" << callId;
            return;
        }
        ...
        qWarning() << "PiP muteAudioToggle: toggling mute from" << muted << "to" << !muted;
        callModel->muteMedia(callId, "audio_0", !muted);
    } catch (const std::exception& e) {
        qWarning() << "PiP muteAudioToggle exception:" << e.what();
    }
}
