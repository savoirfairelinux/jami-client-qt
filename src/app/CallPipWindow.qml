...
Window {
    ...
    function _applyVideoAspect() {
        ...
        if (aspectChanged) {
            ...
            // If height would be clamped, recompute width from the clamped
            // height so both dimensions honour the minimum-size constraints
            // and the aspect ratio is still preserved.
            if (newHeight < minimumHeight) {
                newHeight = minimumHeight;
                newWidth = Math.max(minimumWidth, Math.round(newHeight / r));
            }
        } else {
        ...
        if (Math.abs(newWidth - width) <= 1 && Math.abs(newHeight - height) <= 1) {
            // Geometry is already correct — commit the cached aspect now
            // so future calls treat it as up-to-date.
            _lastInvAspect = r;
            _lastInvAspectInit = true;
            return;
        }
        // Apply geometry via the adjustment guard so onWidthChanged /
        // onHeightChanged do not re-enter the opposite resize path.
        // The cached aspect is updated only after the assignment so that
        // if the compositor refuses or clamps the request the next
        // onVideoInvAspectRatioChanged will retry.
        root._setAspectGeometry(newWidth, newHeight);
        _lastInvAspect = r;
        _lastInvAspectInit = true;
    }
    ...
    // onHeightChanged do not re-enter the opposite resize path.
    property bool _adjustingAspect: false

    function _setAspectGeometry(newWidth, newHeight) {
    ...
    // Adjust width to honour the video aspect ratio after a height-only
    // resize (e.g. user drags the top or bottom window edge).
    function _applyVideoAspectFromHeight() {
        if (!_restored)
            return;
        const r = content.videoInvAspectRatio;
        if (r <= 0)
            return;
        const newWidth = Math.max(minimumWidth, Math.round(height / r));
        if (Math.abs(newWidth - width) <= 1)
            return;
        root._setAspectGeometry(newWidth, height);
    }
