...
void settingsCallback(GSettings* self, gchar* key, gpointer user_data) {
...
UtilsAdapter::isSystemDark()
{
    ...
    if (colorSchemeCStr) {
        ...
        if (!colorScheme.isEmpty()) {
            return colorScheme.contains("dark", Qt::CaseInsensitive) || colorScheme.contains("black", Qt::CaseInsensitive);
        }
    }
    if (gtkThemeCStr) {
        // Fallback on gtk-theme if no color-scheme prefenrece
        QString gtkTheme = gtkThemeCStr;
        if (!gtkTheme.isEmpty()) {
            return gtkTheme.contains("dark", Qt::CaseInsensitive) || gtkTheme.contains("black", Qt::CaseInsensitive);
        }
    }
