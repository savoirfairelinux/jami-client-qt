...
QtObject {
    ...
    // This state needs to be toggled because the SidePanel content is replaced.
    // This makes it so the state can't be inferred from loaded views in single
    // pane mode.
    property bool inSettings: {
        (sv1.currentItem && sv1.currentItem.objectName === "SettingsView") ||
        (sv2.currentItem && sv2.currentItem.objectName === "SettingsView")
    }
