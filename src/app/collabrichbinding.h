...
public:
    ...
    /// Whether a file of suffix @p suffix can be written by this build. All but
    /// PDF are written by QTextDocumentWriter, whose formats Qt can be built
    /// without, so a caller offering a choice of formats is expected to ask
    /// rather than offer one that would fail at the last moment.
    Q_INVOKABLE bool canExportAs(const QString& suffix) const;
