...
Rectangle {
    ...
    // property bool forceAuthPage: false
    // property bool readyForAuth: false // this is to handle the case where the backend jumps over the connectincg states and makes a transition akin to HANDSHAKE -> DATA?
    ...
    Connections {
        ...
        function onDeviceAuthStateChanged(accountId, state, detail) {
            ...
            case 0: {// show qr
                ...
                // root.showThisPage()
                break
                // note: this state can transition to a dht failure because it precedes the dht announcement callback
                // it does however make the UI faster for the link device interaction
            }
            case 1: {// token avail
                // set the uri
                root.updateUri(detail);
            ...
            case 2: {// connecting
                // console.warn("[LinkDevice] ImportFromDevicePage: double checking archive_with_auth signal: ", detail);
                // if (!root.forceAuthPage)
            ...
            case 3: {// auth state
                ...
                case "invalid_credentials":
                    console.warn("[Linkdevice] STATE 3 wrong password");
                    WizardViewStepModel.jumpToAuthLinkDevice();
                    break
                }
                break
