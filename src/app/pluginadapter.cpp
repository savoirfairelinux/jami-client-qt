...
PluginAdapter::getPluginsFromStore()
{
    ...
    pluginVersionManager_
        ->sendGetRequest(QUrl(baseUrl_ + "?arch=" + lrcInstance_->pluginModel().getPlatformInfo()["os"]),
...
{
    return baseUrl_ + "/icons/" + pluginId + "?arch=" + lrcInstance_->pluginModel().getPlatformInfo()["os"];
}

QString
PluginAdapter::getBackgroundImageUrl(const QString& pluginId) const
{
    return baseUrl_ + "/backgrounds/" + pluginId + "?arch=" + lrcInstance_->pluginModel().getPlatformInfo()["os"];
}
