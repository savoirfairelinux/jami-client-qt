...
VideoProvider::onFrameUpdated(const QString& id)
{
    ...
    // This section is only for shared memory rendering mode.
    if (!avModel_.useDirectRenderer()) {
        auto* videoFrame = it->second->getMappedFrame(QVideoFrame::WriteOnly);
        if (!videoFrame) {
            return;
        }
    ...
    // QVideoSink::setVideoFrame also checks size and format of the frame, and
    // notifies the subscribers if they change.
    if (!it->second->swapFrames()) {
        // If the frames cannot be mapped/swapped, we skip this frame.
        return;
    }
    auto* frameToUpload = it->second->getFrame(QVideoFrame::ReadOnly);
    if (frameToUpload->isMapped()) {
        frameToUpload->unmap();
    }
    QReadLocker subsLk(&it->second->subscribersMutex);
    for (const auto& sink : qAsConst(it->second->subscribers)) {
        sink->setVideoFrame(*frameToUpload);
    }
}
