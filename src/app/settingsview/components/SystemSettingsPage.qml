    ...
    flickableContent: ColumnLayout {
        ...
        Column {
            ...
            ToggleSwitch {
                ...
                checked: new Date(Date.parse(new Date(2998, 1, 1, 0, 0, 0, 0).toISOString().slice(0, 16).replace("T", " "))) >= new Date(Date.parse(UtilsAdapter.getAppValue(Settings.Key.DonateDate)))
                labelText: JamiStrings.enableDonation
                tooltipText: JamiStrings.enableDonation
                onSwitchToggled: {
                    if (checked) {
                        UtilsAdapter.setAppValue(Settings.Key.DonateDate, new Date(new Date().getTime() - 24 * 60 * 60 * 1000).toISOString().slice(0, 16).replace("T", " "));
                    } else {
                        UtilsAdapter.setAppValue(Settings.Key.DonateDate, new Date(2999, 1, 1, 0, 0, 0, 0).toISOString().slice(0, 16).replace("T", " "));
                    }
                }
            }
