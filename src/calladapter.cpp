    ...
    , systemTray_(systemTray)
{
    ...
    participantsModelFiltered_.reset(new GenericParticipantsFilterModel(lrcInstance_, participantsModel_.get()));
    QML_REGISTERSINGLETONTYPE_POBJECT(NS_MODELS, participantsModelFiltered_.get(), "GenericParticipantsFilterModel");
    activeParticipantsModel_.reset(new ActiveParticipantsFilterModel(lrcInstance_, participantsModel_.get()));
    QML_REGISTERSINGLETONTYPE_POBJECT(NS_MODELS, activeParticipantsModel_.get(), "ActiveParticipantsFilterModel");
...
CallAdapter::onCallInfosChanged(const QString& accountId, const QString& callId)
{
    ...
    try {
        ...
        if (!convInfo.uid.isEmpty()) {
            QVariantList var {};
            var.push_back(call.isAudioOnly);
            Q_EMIT callInfosChanged(var, accountId, convInfo.uid);
