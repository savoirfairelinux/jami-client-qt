...
CallAdapter::isCurrentHost() const
{
    ...
    if (!convInfo.uid.isEmpty()) {
        ...
        try {
            ...
            auto call = callModel->getCall(confId);
            if (call.participantsInfos.size() == 0) {
                qDebug()<<"call.participantsInfossize == 0";
                return true;
            } else {
                qDebug()<<"convInfo.confId"<<convInfo.confId<<(!convInfo.confId.isEmpty() && callModel->hasCall(convInfo.confId));
...
CallAdapter::participantIsHost(const QString& uri) const
{
    ...
    if (!convInfo.uid.isEmpty()) {
        ...
        try {
            if (isCurrentHost()) {
                qDebug()<<"isCurrentHost"<<(uri == accInfo.profileInfo.uri);
                return uri == accInfo.profileInfo.uri;
            } else {
                auto call = callModel->getCall(convInfo.callId);
                auto peer = call.peerUri.remove("ring:");
                qDebug()<<convInfo.confId<<uri<<peer;
                qDebug()<<(!convInfo.confId.isEmpty()
