...
CallAdapter::fillParticipantData(QMap<QString, QString> participant)
{
    ...
    qDebug() << bestName
             << (participant["audioLocalMuted"] == "true")
             << (participant["audioModeratorMuted"] == "true");
...
{
    qDebug()<<"CallAdapter::getConferencesInfos()";
    QVariantList map;
    auto* convModel = LRCInstance::getCurrentConversationModel();
    const auto convInfo = convModel->getConversationForUID(convUid_);
    if (convInfo.uid.isEmpty())
...
CallAdapter::updateCallOverlay(const lrc::api::conversation::Info& convInfo)
{
    ...
    qDebug() << "CallAdapter::updateCallOverlay" << isAudioMuted;
...
{
    qDebug () << "muteParticipant" << uri << state;
    auto* callModel = LRCInstance::getAccountInfo(accountId_).callModel.get();
    auto* convModel = LRCInstance::getCurrentConversationModel();
    const auto conversation = convModel->getConversationForUID(LRCInstance::getCurrentConvUid());
    auto confId = conversation.confId;
