    ...
    , systemTray_(systemTray)
{
    ...
    connect(systemTray_, &SystemTray::answerCallActivated, [this](const QString& convUid) {
        acceptACall(lrcInstance_->getCurrentAccountInfo().id, convUid);
    });
    connect(systemTray_, &SystemTray::declineCallActivated, [this](const QString& convUid) {
        refuseACall(lrcInstance_->getCurrentAccountInfo().id, convUid);
    });
}
