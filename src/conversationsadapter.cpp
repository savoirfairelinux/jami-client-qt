...
{
    qDebug()<<"selectConversation3"<<convUid;
...
{
    qDebug()<<"selectConversation1"<<convUid;
...
ConversationsAdapter::selectConversation(const lrc::api::conversation::Info& convInfo,
                                         bool preventSendingSignal)
{
    qDebug()<<"selectConversation2"<<convInfo.uid;
    ...
    qDebug()<<LRCInstance::getCurrentConvUid()<<convInfo.uid;
    qDebug()<<LRCInstance::getCurrAccId()<<convInfo.accountId;
...
{
    qDebug()<<"showConversation1"<<convUid;
    auto* convModel = LRCInstance::getCurrentConversationModel();
    if (convModel == nullptr) {
        qDebug()<<"shit";
        return;
    ...
    if (!conversation.uid.isEmpty()) {
        qDebug()<<"showconversation:: showChatView";
        emit showChatView(LRCInstance::getCurrAccId(), conversation.uid);
    }
}
