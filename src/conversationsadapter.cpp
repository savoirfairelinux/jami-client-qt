        ...
        if (convUid.isEmpty()) {
            ...
            try {
                ...
                if (contact.profileInfo.type == profile::Type::SIP
                    || contact.profileInfo.type == profile::Type::RING)
                                             ...
                                             const interaction::Info& interaction)
{
    if (!interaction.authorUri.isEmpty()
        ...
        auto onClicked = [this, accountId, convUid, uri = interaction.authorUri] {
            ...
            if (!convInfo.uid.isEmpty()) {
                lrcInstance_->selectConversation(accountId, convInfo.uid);
            }
        };
    ...
    if (!convInfo.uid.isEmpty() && !convInfo.participants.isEmpty()) {
        try {
            ...
            if (!contact.profileInfo.uri.isEmpty()
                ...
                lrcInstance_->selectConversation(convInfo.accountId, convUid);
                convModel_->selectSourceRow(lrcInstance_->indexOf(convUid));
            }
        } catch (...) {
            return;
