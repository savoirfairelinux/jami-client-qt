...
QImage
Utils::contactPhoto(const QString& contactUri, const QSize& requestedSize)
{
    QImage photo;
    ...
    try {
        ...
        if (accountInfo.profileInfo.type == lrc::api::profile::Type::SIP
            ...
            photo = contactPhotoFromBase64ByteArray(byteArray, nullptr);
            if (photo.isNull()) {
    ...
    return Utils::scaleAndFrame(photo, requestedSize);
}

QImage
Utils::contactPhotoFromBase64ByteArray(const QByteArray& data, const QString& type)
{
    QImage avatar;
    const bool ret = avatar.loadFromData(QByteArray::fromBase64(data), type.toLatin1());
    if (!ret) {
        qDebug() << "AvatarImageProvider: vCard image loading failed";
        return QImage();
    }
...
Utils::accountPhoto(const lrc::api::account::Info& accountInfo, const QSize& size)
{
    ...
    if (!accountInfo.profileInfo.avatar.isEmpty()) {
        ...
        photo = contactPhotoFromBase64ByteArray(ba, nullptr);
    } else {
