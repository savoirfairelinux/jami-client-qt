...
{
    // TODO: Filter moderators
    if (!parent.isValid()) {
        return LRCInstance::getCurrentAccountInfo().contactModel->getAllContacts().size();
    }
    return 0;
}
...
{
    // TODO: Filter moderators
    auto allContacts = LRCInstance::getCurrentAccountInfo().contactModel->getAllContacts();
    QList<QString> list {};
    for (auto i = allContacts.begin(); i != allContacts.end(); ++i) {
        list.append(i.value().profileInfo.uri);
    }
