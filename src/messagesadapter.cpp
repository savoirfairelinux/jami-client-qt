...
QVariantMap
MessagesAdapter::getMediaDesc(const QString& msg)
{
    if (isLocalImage(msg)) {
        return {{"isImage", true}};
    }
    QRegularExpression vPattern("[^\\s]+(.*?)\\.(mp4|avi|mov|webm|webp|rmvb)$",
                                QRegularExpression::CaseInsensitiveOption);
    QRegularExpressionMatch vMatch = vPattern.match(msg);
    QString type = vMatch.captured(2);
    qDebug() << vMatch.captured(0);
    if (!type.isEmpty()) {
        return {{"video", type}};
    } else {
        QRegularExpression aPattern("[^\\s]+(.*?)\\.(ogg|flac|wav|mpeg|mp3)$",
                                    QRegularExpression::CaseInsensitiveOption);
        type = aPattern.match(msg).captured(2);
        if (!type.isEmpty()) {
            return {{"audio", type}};
        }
    }
