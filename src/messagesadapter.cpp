...
MessagesAdapter::isUrl(QString urlInputted)
{
    ...
    return std::regex_match(
        url, std::regex("^(https?:\\/\\/)?([\\da-z\\.-]+)\\.([a-z\\.]{2,6})([\\/\\w \\.-]*)*\\/?$"));
}
...
void
MessagesAdapter::onMessageLinkified(const QString& messageId, const QString& linkified, bool hasUrl)
{
    const QString& convId = lrcInstance_->get_selectedConvUid();
    const QString& accId = lrcInstance_->get_currentAccountId();
    auto& conversation = lrcInstance_->getConversationFromConvUid(convId, accId);
    conversation.interactions->linkifyMessage(messageId, linkified, hasUrl);
}

bool
MessagesAdapter::isImage(const QString& message)
{
    QRegularExpression pattern("[^\\s]+(.*?)\\.(jpg|jpeg|png|JPG|JPEG|PNG)$");
