...
void
MessagesAdapter::onMessageLinkified(QString messageId, QString linkifiedMessage, bool urlPresent)
{
    const QString& convId = lrcInstance_->get_selectedConvUid();
    const QString& accId = lrcInstance_->get_currentAccountId();
    auto& conversation = lrcInstance_->getConversationFromConvUid(convId, accId);
    conversation.interactions->addLinkifiedMessage(messageId, linkifiedMessage, urlPresent);
}
