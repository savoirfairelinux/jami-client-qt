...
MessagesAdapter::setupChatView(const QVariantMap& convInfo)
{
    ...
    if (convInfo["isSwarm"].toBool()) {
        auto id = convModel->loadConversationMessages(convId, loadChunkSize_);
        if (id != -1) {
            set_msgRequestPending(true);
            std::get<0>(msgRequest_) = id;
...
MessagesAdapter::loadMoreMessages()
{
    ...
    if (convInfo.isSwarm()) {
        ...
        if (id != -1) {
            ...
            msgRequest_ = std::make_tuple(id, MsgRequestType::Supplement, rowCount);
        }
    }
}
