...
public:
    ...
    void synchronize(QQuickFramebufferObject* item) override
    {
        ...
        if (i->getRenderingFinished()) {
            // destroy() will destroy initial settings for texture as well
            render_->clearTextures();
            render_->initializeTextures();
...
void
TexturesRenderer::initializeTextures()
{
...
TexturesRenderer::processAllTexturesData()
{
    ...
    if (frame_->linesize[0] && frame_->linesize[1] && frame_->linesize[2]) {
        ...
        if (AVPixelFormat(frame_->format) == AVPixelFormat::AV_PIX_FMT_YUV420P
            ...
            // Y
            processTextureDataYUV(Ytex_.get(), TexturePlane::Y);
            // U
            processTextureDataYUV(Utex_.get(), TexturePlane::U);
            // V
            processTextureDataYUV(Vtex_.get(), TexturePlane::V);
        }
    }
    ...
    if (frame_->linesize[0] && frame_->linesize[1]) {
        ...
        if (AVPixelFormat(frame_->format) == AVPixelFormat::AV_PIX_FMT_NV12) {
            ...
            processTextureDataNV12();
        }
    }
}

void
TexturesRenderer::processTextureDataYUV(QOpenGLTexture* texture, TexturePlane plane)
{
    if (!texture->isStorageAllocated()) {
        texture->create();
        texture->setSize((plane == TexturePlane::Y ? 1
                          : (AVPixelFormat(frame_->format) == AVPixelFormat::AV_PIX_FMT_YUV444P)
                              ? 1
                              : 0.5)
                             * frame_->width,
                         (plane == TexturePlane::Y ? 1
        ...
        option.setRowLength(frame_->linesize[static_cast<int>(plane)]);
        options_.insert(plane, option);
    ...
    texture->setData(0,
                     ...
                     frame_->data[static_cast<int>(plane)],
                     &options_[plane]);
}

void
TexturesRenderer::processTextureDataNV12()
{
    if (!UVtex_NV12_->isStorageAllocated()) {
        UVtex_NV12_->create();
        UVtex_NV12_->setSize(frame_->width / 2, frame_->height / 2);

        QOpenGLPixelTransferOptions option;
        option.setAlignment(1);
        option.setRowLength(frame_->linesize[1] / 2);
        options_.insert(TexturePlane::NV12, option);

        UVtex_NV12_->allocateStorage(QOpenGLTexture::LuminanceAlpha,
                                     QOpenGLTexture::PixelType::UInt8);
    }

    UVtex_NV12_->setData(0,
                         0,
                         QOpenGLTexture::LuminanceAlpha,
                         QOpenGLTexture::PixelType::UInt8,
                         frame_->data[1],
                         &options_[TexturePlane::NV12]);
}
...
TexturesRenderer::clearTextures()
{
    ...
    if (Ytex_ && Ytex_->isStorageAllocated()) {
        Ytex_->destroy();
    }
    if (Utex_ && Utex_->isStorageAllocated()) {
        Utex_->destroy();
    }
    if (Vtex_ && Vtex_->isStorageAllocated()) {
        Vtex_->destroy();
    }
    if (UVtex_NV12_ && UVtex_NV12_->isStorageAllocated()) {
        UVtex_NV12_->destroy();
    }
}
