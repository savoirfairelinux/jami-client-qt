...
TexturesRenderer::processAllTexturesData()
{
    ...
    auto newFrameSize = QSize(frame_->width, frame_->height);
    if (textureSize_ != newFrameSize) {
        textureSize_ = newFrameSize;
    ...
    if (frame_->linesize[0] && frame_->linesize[1] && frame_->linesize[2]) {
        ...
        if (AVPixelFormat(frame_->format) == AVPixelFormat::AV_PIX_FMT_YUV420P
            ...
            // Y
            processTextureDataYUV(Ytex_.get(),
                                  frameSizeChanged,
                                  frame_->linesize[0],
                                  frame_->height,
                                  frame_->data[0]);
...
void
TexturesRenderer::processTextureDataYUV(
    QOpenGLTexture* texture, bool frameSizeChanged, int width, int height, uint8_t* data)
{
    if (frameSizeChanged) {
        texture->create();
...
{
    textureSize_.setWidth(0);
    textureSize_.setHeight(0);
