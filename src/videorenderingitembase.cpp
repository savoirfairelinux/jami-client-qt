...
TexturesRenderer::initializeShaderProgram()
{
    if (!shaderProgram_) {
        ...
        angleToRotateZId_ = shaderProgram_->uniformLocation("aAngleToRotateZ");
        angleToRotateYId_ = shaderProgram_->uniformLocation("aAngleToRotateY");
...
TexturesRenderer::initializeAllTextures()
{
    ...
    if (renderType_ == VideoRenderingItemBase::Type::DISTANT) {
        ...
        if (auto matrix = av_frame_get_side_data(frame_.get(), AV_FRAME_DATA_DISPLAYMATRIX)) {
            ...
            angleToRotateZ_ = angle != 0 ? angle : angleToRotateZ_;
        }
    } else {
        angleToRotateZ_ = 180.0;
        angleToRotateY_ = 0.0;
