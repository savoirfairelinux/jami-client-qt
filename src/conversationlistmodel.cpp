...
ConversationListProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
{
    ...
    auto mode = static_cast<conversation::Mode>(index.data(Role::Mode).toInt());
    toFilter += index.data(Role::UriList).toString();
    if (mode == conversation::Mode::NON_SWARM) {
        toFilter += index.data(Role::Alias).toString();
        toFilter += index.data(Role::RegisteredName).toString();
    } else {
