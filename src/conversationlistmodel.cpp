...
ConversationListModel::itemFromIndex(const QModelIndex& index) const
{
...
{
    setSortRole(Conversation::Role::LastInteractionTimeStamp);
...
ConversationListProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
{
    ...
    auto itemProfileType = index.data(Conversation::Role::ContactType).toInt();
    auto displayName = index.data(Conversation::Role::DisplayName).toString();
    auto displayId = index.data(Conversation::Role::DisplayID).toString();
