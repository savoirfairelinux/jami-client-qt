    ...
    : QAbstractListModel(parent)
{
    ...
    if (config) {
        connect(config, &ConfigurationManagerInterface::swarmLoaded, this, &MessageListModel::onSwarmLoaded);
        connect(config,
                &ConfigurationManagerInterface::conversationLoaded,
                this,
                &MessageListModel::onConversationLoaded);
        connect(config, &ConfigurationManagerInterface::messagesFound, this, &MessageListModel::onConversationLoaded);
        connect(config,
                &ConfigurationManagerInterface::swarmMessageReceived,
                this,
                &MessageListModel::onSwarmMessageReceived);
    }
}
...
QString
MessageListModel::accountId() const
{
    return m_accountId;
}

void
MessageListModel::setAccountId(const QString& accountId)
{
    qDebug() << "[MessageListModel] setAccountId:" << accountId;
...
QString
MessageListModel::conversationId() const
{
    return m_conversationId;
}

void
MessageListModel::setConversationId(const QString& conversationId)
{
    qDebug() << "[MessageListModel] setConversationId:" << conversationId;
...
MessageListModel::loadMessages()
{
    ...
    if (m_accountId.isEmpty() || m_conversationId.isEmpty()) {
        qDebug() << "[MessageListModel] loadMessages ABORT: Empty ID. Acc:" << m_accountId
                 << "Conv:" << m_conversationId;
        return;
    }
    ...
    auto* config = CoreService::instance().configurationManager();
    if (config) {
        // Clear cache to force reload of history
        config->clearCache(m_accountId, m_conversationId);
...
void
MessageListModel::onConversationLoaded(uint32_t requestId,
                                       const QString& accountId,
                                       const QString& conversationId,
                                       const VectorMapStringString& messages)
{
    qDebug() << "[MessageListModel] RCV ConversationLoaded. Acc:" << accountId << "Conv:" << conversationId
             << "Count:" << messages.size();
...
void
MessageListModel::onSwarmLoaded(uint32_t requestId,
                                const QString& accountId,
                                const QString& conversationId,
                                const VectorSwarmMessage& messages)
{
    qDebug() << "[MessageListModel] RCV SwarmLoaded. Acc:" << accountId << "Conv:" << conversationId
             << "Count:" << messages.size();
    ...
    for (const auto& msg : messages) {
        ...
        qDebug() << "[MessageListModel] Parsed Msg:" << item.id << " Body:" << info.body
                 << " Type:" << lrc::api::interaction::to_string(info.type);
...
void
MessageListModel::onSwarmMessageReceived(const QString& accountId,
                                         const QString& conversationId,
