    ...
    : QAbstractListModel(parent)
{
    ...
    if (config) {
        connect(config, &ConfigurationManagerInterface::conversationReady,
                this, &ConversationListModel::onConversationReady);
        connect(config, &ConfigurationManagerInterface::conversationRemoved,
                this, &ConversationListModel::onConversationRemoved);
        ...
        // void conversationProfileUpdated(const QString& accountId, const QString& conversationId, const MapStringString& profile);
        
        connect(config, &ConfigurationManagerInterface::conversationProfileUpdated,
                this, [this](const QString& accId, const QString& convId, const MapStringString& profile){
...
QString ConversationListModel::accountId() const
{
    return m_accountId;
}

void ConversationListModel::setAccountId(const QString& accountId)
{
    if (m_accountId == accountId) return;
...
void ConversationListModel::onConversationReady(const QString& accountId, const QString& conversationId)
{
    if (accountId != m_accountId) return;
    reload(); // Naive implementation: full reload. Ideally just insert one.
}

void ConversationListModel::onConversationRemoved(const QString& accountId, const QString& conversationId)
{
    if (accountId != m_accountId) return;
    reload(); // Naive implementation
}

void ConversationListModel::onConversationProfileUpdated(const QString& accountId, const QString& conversationId, const QVariantMap& profile)
{
    if (accountId != m_accountId) return;
...
QString ConversationListModel::getConversationId(int row)
{
