...
QObject*
CoreService::create(QQmlEngine* qmlEngine, QJSEngine* jsEngine)
{
    Q_UNUSED(qmlEngine);
    Q_UNUSED(jsEngine);
...
bool
CoreService::isInitialized() const
{
    return m_initialized;
}

ConfigurationManagerInterface*
CoreService::configurationManager() const
{
    return m_configManager;
}

CallManagerInterface*
CoreService::callManager() const
{
    return m_callManager;
}

void
CoreService::setupConnections()
{
    if (!m_configManager)
        return;

    // Connect raw backend signals to CoreService signals
    connect(m_configManager, &ConfigurationManagerInterface::accountsChanged, this, &CoreService::accountsChanged);
...
QStringList
CoreService::getAccountList() const
{
    if (!m_configManager)
        return {};
...
QVariantMap
CoreService::getAccountDetails(const QString& accountId) const
{
    if (!m_configManager)
        return {};
