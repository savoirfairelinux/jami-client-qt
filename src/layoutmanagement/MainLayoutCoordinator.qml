...
Item {
    ...
    property bool inSettingsView: {
        if (initialized) {
            var itemName = leftStackView.currentItem.objectName
            return itemName === "settingsMenu" || itemName === "settingsView"
        }
        return false
    }
    ...
    function setMainView(currentUserDisplayName, currentUserAlias, currentUID,
        ...
        var communicationPageMessageWebView = views.get("communicationPageMessageWebView")
        var callStackView = views.get("callStackView")
        ...
        if (callStackViewShouldShow) {
            ...
            } else if (isPageInStack("callStackView", leftStackView)
    ...
    function pushCallStackView() {
        if (sidePanelOnly) {
            leftStackView.pop(StackView.Immediate)
            leftStackView.push(views.get("callStackView"), StackView.Immediate)
        } else {
            leftStackView.pop(StackView.Immediate)
            rightStackView.pop(views.get("welcomePage"), StackView.Immediate)
            rightStackView.push(views.get("callStackView"), StackView.Immediate)
        }
    }
    ...
    function mainViewWidthChanged(currentWidth) {
        ...
        if (currentWidth < widthToCompare - JamiTheme.onWidthChangedTriggerDistance
            ...
            if (inSettingsView) {
                ...
                if (UtilsAdapter.hasCall(AccountAdapter.currentAccountId))
                    pushCallStackView()
            }
        }
    }

    function changeSelectedSettings(menuType) {
        var settingsView = views.get("settingsView")
        if (settingsView.selectedMenu !== menuType)
            settingsView.setSelected(menuType)
        if (sidePanelOnly)
            leftStackView.push(settingsView, StackView.Immediate)
    }

    function showSettingsMenu() {
        leftStackView.pop(views.get("settingsMenu"), StackView.Immediate)
    }
    ...
    Connections {
        ...
        function onCurrentAccountIdChanged() {
            ...
            if (MainLayoutCoordinator.inSettingsView) {
                ...
                settingsView.setSelected(settingsView.selectedMenu)
            } else {
                backToMainView(true)
            }
        }
