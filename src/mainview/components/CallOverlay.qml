...
Rectangle {
    ...
    function isFullyMaximized(x, y, w, h) {
        return (x - distantRenderer.getXOffset() === 0
                && y - distantRenderer.getYOffset() === 0
                && w === distantRenderer.width - distantRenderer.getXOffset() * 2
                && h === distantRenderer.height - distantRenderer.getYOffset() * 2)
    }
        ...
        if (infos.length === 0) {
            ...
            for (var infoVariant in infos) {
                ...
                if (!currentUris.includes(infos[infoVariant].uri)) {
                    ...
                    isMaxSize = isFullyMaximized(infos[infoVariant].x,
                                                 infos[infoVariant].y,
                                                 infos[infoVariant].width,
                                                 infos[infoVariant].height)
