...
//        conversationListView.currentIndex = -1
    ...
    ColumnLayout {
        ...
        ConversationListView {
            ...
            model: SearchResultsListProxyModel
            delegate: SmartListItemDelegate {}
            headerLabel: JamiStrings.searchResults
            headerVisible: visible

            Connections {
                target: SearchResultsListProxyModel

                // actually select the conversation
                // TODO: make this generic to the model
                function onValidSelectionChanged() {
                    var row = SearchResultsListProxyModel.currentFilteredRow
                    var uid = SearchResultsListProxyModel.dataForRow(row, ConversationListModel.UID)
                    ConversationsAdapter.selectConversation(AccountAdapter.currentAccountId, uid, false)
                }
            }
        }
...
//                // TODO: make this generic to the model
//                function onValidSelectionChanged() {
//                    var row = conversationListView.model.currentFilteredRow
