            ...
            Loader {
                ...
                sourceComponent: ListView {
                    ...
                    delegate: MessageDelegate {
                        ...
                        function printStuff(when) {
                            if (seq !== MsgSeq.unknown)
                                return
                            seq = MsgSeq.single
                            var pItem = chatView.itemAtIndex(index + 1)
                            var nItem = chatView.itemAtIndex(index - 1)
                            print(when, index, chatView.count,
                                  body,
                                  "UP1:", pItem ? pItem.body : "NULL",
                                  " DWN1:", nItem ? nItem.body : "NULL")
                            computeSequencing()
                        }

                        Component.onCompleted: {
                            Qt.callLater(printStuff, "comp")
                        }
                        ListView.onAdd: {
                            Qt.callLater(printStuff, "add")
                        }
                    }
