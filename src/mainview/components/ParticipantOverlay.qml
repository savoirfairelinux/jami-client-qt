...
Rectangle {
    ...
    property bool participantIsModerator: overlayMenu.participantIsModerator
    ...
    function setMenu(isModerator, isHost, setUri, bestName, active, isLocal) {
        var layout = CallAdapter.getCurrentLayoutType()
        var showMaximized = layout !== 2
        var showMinimized = !(layout === 0 || (layout === 1 && !active))

        overlayMenu.uri = setUri
        overlayMenu.bestName = bestName
        overlayMenu.active = active
        overlayMenu.isModerator = isModerator // local is moderator
        overlayMenu.participantIsModerator = CallAdapter.isModerator(overlayMenu.uri)
        ...
        overlayMenu.isMuted = CallAdapter.isMuted(overlayMenu.uri)
    ...
    Rectangle {
        ...
        height: shapeHeight
        visible: participantIsModerator || overlayMenu.isMuted
        color: "transparent"
    ...
    Rectangle {
        ...
        MouseArea {
            ...
            onClicked: {
                CallAdapter.maximizeParticipant(overlayMenu.uri, overlayMenu.active)
            }
