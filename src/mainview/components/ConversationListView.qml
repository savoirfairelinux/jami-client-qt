...
ListView {
    ...
    function openContextMenuAt(x, y, delegate) {
        contextMenu.parent = delegate
        contextMenu.openMenuAt(x, y)
    }
    ...
    ConversationSmartListContextMenu {
        ...
        function openMenuAt(x, y) {
            ...
            // - userProfile dialog should use a loader

            var row = root.indexAt(x, y)
            var item = {
                "convId": model.dataForRow(row, ConversationListModel.UID),
                "displayId": model.dataForRow(row, ConversationListModel.DisplayID),
                "displayName": model.dataForRow(row, ConversationListModel.DisplayName),
                "uri": model.dataForRow(row, ConversationListModel.URI),
                "contactType": model.dataForRow(row, ConversationListModel.ContactType),
            }
