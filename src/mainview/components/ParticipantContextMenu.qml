...
Menu {
    ...
    property var visibleItems: 1

    function showMazimize(show) {
        if (show && !maximizeItem.visible) {
            maximizeItem.visible = true
            maximizeItem.height = hangupItem.height
            visibleItems += 1
        } else if (maximizeItem.visible) {
    ...
    function showMinimize(show) {
        if (show && !minimizeItem.visible) {
            minimizeItem.visible = true
            minimizeItem.height = hangupItem.height
            visibleItems += 1
        } else if (minimizeItem.visible) {
    ...
    background: Rectangle {
        ...
        implicitHeight: hangupItem.preferredHeight * (root.visibleItems)
