...
def run_tests(mutedring):
    print('Running client tests')

    test_exe_command_list = []

    if mutedring:
        test_exe_command_list = [qml_test_exe + ' -mutedring', unit_test_exe + ' -mutedring']
    else:
        test_exe_command_list = [qml_test_exe, unit_test_exe]

    for test_exe_command in test_exe_command_list:
        if (execute_cmd(test_exe_command, True, env_vars={'QT_QPA_PLATFORM' : 'offscreen'})):
            print("Tests failed")
            sys.exit(1)
...
def parse_args():
    ...
    run_test = subparser.add_parser('runtest')
...
def main():
    ...
    if parsed_args.subparser_name == 'runtest':
        run_tests(parsed_args.mutedring)
