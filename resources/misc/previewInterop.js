...
function parseMessage(messageId, message)
{
    ...
    window.jsbridge.log("ASODNMAS{PODMJAS}PDMASPDMIASDPIAMDPAMSDPIAMS}DPASMD}APISM")

    window.jsbridge.log(linkifiedString)
    window.jsbridge.messageLinkifyReady(messageId, linkifiedString)

    var linkArray = linkify.find(message)
    if (linkArray.length > 0){
        getPreviewInfo(messageId, link_array[0].href)
    }
}
