...
function parseMessage(messageId, message)
{
    ...
    console.log("ASDDDDDDDDDDDD" + message)
    var linkArray = linkify.find(message)
    if (linkArray.length > 0){
        window.jsbridge.messageLinkifyReady(messageId, linkifiedString, true)
        getPreviewInfo(messageId, linkArray[0].href)
        return
    }
     window.jsbridge.messageLinkifyReady(messageId, linkifiedString, false)
