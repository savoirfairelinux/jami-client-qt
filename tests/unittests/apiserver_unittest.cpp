...
public:
    void SetUp() override
    {
        ...
        // Create a test token via the token manager for authenticating requests.
        auto result = server->tokenManager()->createToken(QString(), "test", {}, 0);
        testToken = result.rawToken;
    }
    ...
    {
        return httpRequest(nam, "GET", QUrl(baseUrl + path), token.isEmpty() ? testToken : token);
    }

    HttpResponse post(const QString& path, const QJsonObject& body = {}, const QString& token = {})
    {
        return httpRequest(nam, "POST", QUrl(baseUrl + path), token.isEmpty() ? testToken : token, body);
    }

    HttpResponse put(const QString& path, const QJsonObject& body = {}, const QString& token = {})
    {
        return httpRequest(nam, "PUT", QUrl(baseUrl + path), token.isEmpty() ? testToken : token, body);
    }

    HttpResponse del(const QString& path, const QString& token = {})
    {
        return httpRequest(nam, "DELETE", QUrl(baseUrl + path), token.isEmpty() ? testToken : token);
    }
    ...
    {
        return httpRequest(nam, "HEAD", QUrl(baseUrl + path), token.isEmpty() ? testToken : token);
    }
...
TEST_F(ApiServerFixture, TokenHasCorrectFormat)
{
    EXPECT_TRUE(testToken.startsWith("jm_sk_"));
    EXPECT_EQ(testToken.length(), 6 + 64);
}
...
TEST_F(ApiServerFixture, RequestWithTokenReturns200)
{
...
TEST_F(ApiServerFixture, WebSocketAcceptsValidToken)
{
    auto wsPort = server->port();
    QWebSocket ws;
    QSignalSpy connectedSpy(&ws, &QWebSocket::connected);

    ws.open(QUrl(QStringLiteral("ws://127.0.0.1:%1/api?accessToken=%2").arg(wsPort).arg(testToken)));
...
TEST_F(ApiServerFixture, WebSocketRejectsLegacyTokenQueryParameter)
{
    ...
    ws.open(QUrl(QStringLiteral("ws://127.0.0.1:%1/api?token=%2").arg(wsPort).arg(testToken)));
...
TEST_F(ApiServerFixture, CanStartMultipleServersOnDifferentPorts)
{
    ...
    // Give server2 its own separate token manager (in-memory only, no shared storage).
    ApiTokenManager tm2;
    tm2.revokeAllTokens(QString());
    server2.setTokenManager(&tm2);
    ASSERT_TRUE(server2.start(0));
    EXPECT_NE(server->port(), server2.port());

    // Create a token for the second server
    auto result2 = tm2.createToken(QString(), "test2", {}, 0);
    auto token2 = result2.rawToken;

    auto resp = httpRequest(nam, "GET", QUrl(QStringLiteral("http://127.0.0.1:%1/api/conversations").arg(server2.port())),
                            token2);
    EXPECT_EQ(resp.statusCode, 200);

    // Token from server1 should NOT work on server2
    ...
    EXPECT_EQ(crossResp.statusCode, 401)
        << "Token from one server should not authenticate on another";
