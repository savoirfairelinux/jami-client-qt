...
TEST_F(MessageParserFixture, TextIsParsedCorrectly)
{
    ...
    globalEnv.messageParser->parseMessage("msgId_01", "This is a **bold** text", true, linkColor);
...
TEST_F(MessageParserFixture, ALinkIsParsedCorrectly)
{
    ...
    globalEnv.messageParser->parseMessage("msgId_02", "https://www.google.com", true, linkColor);
...
TEST_F(MessageParserFixture, EndOfLineCharactersAreParsedCorrectly)
{
    ...
    // Parse a message with a link.
    globalEnv.messageParser->parseMessage("msgId_03", "Text with\n2 lines", true, linkColor);
    ...
    EXPECT_TRUE(messageParserArguments.at(1).typeId() == qMetaTypeId<QString>());
    EXPECT_EQ(messageParserArguments.at(1).toString(), "<p>Text with<br>\n2 lines</p>\n");
}
...
TEST_F(MessageParserFixture, FencedCodeIsParsedCorrectly)
{
    ...
    // Parse a message with a link.
    globalEnv.messageParser->parseMessage("msgId_04", "Text with \n```\ncode\n```", true, linkColor);
    ...
    EXPECT_TRUE(messageParserArguments.at(1).typeId() == qMetaTypeId<QString>());
    EXPECT_EQ(messageParserArguments.at(1).toString(),
              "<style>pre,code{background-color:#f0f0f0;color:#222222;white-space:pre-wrap;}</"
              "style><p>Text with</p>\n<pre><code>code\n</code></pre>\n");
}
