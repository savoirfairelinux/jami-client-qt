    ...
    ListSelectionView {
        ...
        rightPaneItem: ChatView {
            ...
            TestCase {
                ...
                // must not own a private profile. It uses the shared default
                // profile, so opening the map, closing it, and reopening it must
                // always yield the exact same profile instance – proving a single
                // profile exists process-wide (the invariant that avoids the
                // single-process Chromium crash when web views coexist).
                //
                // Reading WebEngineView.profile with no explicit binding returns
                // WebEngineProfile.defaultProfile(); this file therefore needs no
                // static QtWebEngine import (which would break WITH_WEBENGINE=0
                // builds).
