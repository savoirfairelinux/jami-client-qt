...
void main(void) {
    ...
    if(isNV12) {
        ...
        yuv.x = texture2D(Ytex,
                    vec2(vTextureCoord.x * vTextureCoordScalingFactors.x, vTextureCoord.y)).r;
        yuv.y = texture2D(Utex,
                    vec2(vTextureCoord.x * vTextureCoordScalingFactors.y, vTextureCoord.y)).r;
        yuv.z = texture2D(Vtex,
                    vec2(vTextureCoord.x * vTextureCoordScalingFactors.z, vTextureCoord.y)).r;
    }
