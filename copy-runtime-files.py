    ...
    if globalVar.system_name == "Windows":
        ...
        if parsed_args.qtVersion:
            ...
            globalVar.qt_path = "C:\\Qt\\" + globalVar.qt_version + \
                "\\msvc2019_64" if qt_minor_ver >= 15 else "\\msvc2019_64"
    else:
...
def copy_ringtones():
    # ringtones
    copy_to_dir = globalVar.output_path + os.sep + "ringtones"
...
def release_and_copy_translations():
    ...
    for root, dirs, files in os.walk(lrc_ts_path):
        for file in files:
            if file.endswith(".ts"):
                print(bcolors.OKBLUE + execute_cmd(lrelease + " " +
                                                   lrc_ts_path + os.sep + file, True) + bcolors.ENDC)
        for file in files:
            if file.endswith(".qm"):
    ...
    for root, dirs, files in os.walk(client_ts_path):
        for file in files:
            if file.endswith(".ts"):
                print(bcolors.OKBLUE + execute_cmd(lrelease + " " +
                                                   client_ts_path + os.sep + file, True) + bcolors.ENDC)
        for file in files:
            if file.endswith(".qm"):
                print(bcolors.OKBLUE + "Copying translation file: " +
                      file + bcolors.ENDC)
